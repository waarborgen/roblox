...
Totaal: **11160 repos** in **23 categorieën**: **2269** handgecureerd (met Nederlandse uitleg, hieronder) + **8891** in de uitgebreide index (automatisch verzameld en gefilterd, op de categoriepagina's). ⭐ = GitHub-sterren op het moment van verzamelen (sept. 2026).

## Categorieën

- [VFX, graphics & rendering](#vfx-graphics--rendering) — **81** gecureerd + **188** in index ([volledige pagina](categories/vfx.md))
- [Landschap, terrain, water & weer](#landschap-terrain-water--weer) — **56** gecureerd + **154** in index ([volledige pagina](categories/landscape.md))
- [Animatie, IK & rigging](#animatie-ik--rigging) — **71** gecureerd + **285** in index ([volledige pagina](categories/animation.md))
- [Movement, physics & characters](#movement-physics--characters) — **65** gecureerd + **148** in index ([volledige pagina](categories/movement.md))
- [Voertuigen, boten & treinen](#voertuigen-boten--treinen) — **23** gecureerd + **93** in index ([volledige pagina](categories/vehicles.md))
- [Combat, hitboxes & wapens](#combat-hitboxes--wapens) — **57** gecureerd + **282** in index ([volledige pagina](categories/combat.md))
- [AI, NPC's & machine learning](#ai-npcs--machine-learning) — **35** gecureerd + **95** in index ([volledige pagina](categories/ai.md))
- [Procedurele generatie & wiskunde](#procedurele-generatie--wiskunde) — **42** gecureerd + **119** in index ([volledige pagina](categories/procgen.md))
- [Networking & replicatie](#networking--replicatie) — **88** gecureerd + **220** in index ([volledige pagina](categories/networking.md))
- [ECS & game-architectuur](#ecs--game-architectuur) — **79** gecureerd + **330** in index ([volledige pagina](categories/ecs.md))
- [UI & interface](#ui--interface) — **188** gecureerd + **741** in index ([volledige pagina](categories/ui.md))
- [Camera & cutscenes](#camera--cutscenes) — **27** gecureerd + **106** in index ([volledige pagina](categories/camera.md))
- [Audio](#audio) — **17** gecureerd + **162** in index ([volledige pagina](categories/audio.md))
- [VR & motion tracking](#vr--motion-tracking) — **15** gecureerd + **13** in index ([volledige pagina](categories/vr.md))
- [Data, opslag & serialisatie](#data-opslag--serialisatie) — **141** gecureerd + **305** in index ([volledige pagina](categories/data.md))
- [Game-systemen & utilities](#game-systemen--utilities) — **243** gecureerd + **1961** in index ([volledige pagina](categories/systems.md))
- [Security & anticheat](#security--anticheat) — **35** gecureerd + **236** in index ([volledige pagina](categories/security.md))
- [Next-level / experimenteel](#next-level--experimenteel) — **147** gecureerd + **99** in index ([volledige pagina](categories/wild.md))
- [API's & externe integraties](#apis--externe-integraties) — **131** gecureerd + **830** in index ([volledige pagina](categories/apis.md))
- [AI-tools voor Roblox-development](#ai-tools-voor-roblox-development) — **69** gecureerd + **364** in index ([volledige pagina](categories/aitools.md))
- [Tooling & workflow](#tooling--workflow) — **387** gecureerd + **859** in index ([volledige pagina](categories/tooling.md))
- [Studio-plugins](#studio-plugins) — **92** gecureerd + **368** in index ([volledige pagina](categories/plugins.md))
- [Leren, open-source games & collecties](#leren-open-source-games--collecties) — **180** gecureerd + **933** in index ([volledige pagina](categories/learn.md))

## VFX, graphics & rendering

Particles, shaders, lighting, portalen, raytracing en canvas-rendering. (81 gecureerd — nog **188** meer in de [uitgebreide index](categories/vfx.md#uitgebreide-index-188))
...
Terrain-generators, echte kaarten, grotten, oceanen, foliage, dag/nacht en weer. (56 gecureerd — nog **154** meer in de [uitgebreide index](categories/landscape.md#uitgebreide-index-154))
...
Animatie-solvers, IK, Blender/Mixamo-pipelines, procedurele animatie en springs. (71 gecureerd — nog **285** meer in de [uitgebreide index](categories/animation.md#uitgebreide-index-285))
...
Character controllers, ragdolls, destructie, parkour en eigen physics-engines. (65 gecureerd — nog **148** meer in de [uitgebreide index](categories/movement.md#uitgebreide-index-148))
...
Chassis, suspensie, hover, raketten, drijfvermogen en treinen. (23 gecureerd — nog **93** meer in de [uitgebreide index](categories/vehicles.md#uitgebreide-index-93))
...
Melee-hitboxes, projectielen, guns, lag-compensatie en zones. (57 gecureerd — nog **282** meer in de [uitgebreide index](categories/combat.md#uitgebreide-index-282))
...
Neurale netwerken, behavior trees, pathfinding, boids en LLM-NPC's. (35 gecureerd — nog **95** meer in de [uitgebreide index](categories/ai.md#uitgebreide-index-95))
...
Voxels, dungeons, doolhoven, wave function collapse, noise en curves. (42 gecureerd — nog **119** meer in de [uitgebreide index](categories/procgen.md#uitgebreide-index-119))
...
Buffer-netcode, IDL-compilers, encryptie en eigen replicatie. (88 gecureerd — nog **220** meer in de [uitgebreide index](categories/networking.md#uitgebreide-index-220))
...
Entity Component Systems en frameworks voorbij Knit. (79 gecureerd — nog **330** meer in de [uitgebreide index](categories/ecs.md#uitgebreide-index-330))
...
Reactieve UI, custom tekst, inventories, 2D-engines en mobiele controls. (188 gecureerd — nog **741** meer in de [uitgebreide index](categories/ui.md#uitgebreide-index-741))
...
Camera-shake, custom camera's, cutscene-tools en timelines. (27 gecureerd — nog **106** meer in de [uitgebreide index](categories/camera.md#uitgebreide-index-106))
...
3D-audio, occlusie/reverb, adaptieve muziek en radio. (17 gecureerd — nog **162** meer in de [uitgebreide index](categories/audio.md#uitgebreide-index-162))
...
Full-body VR, SteamVR-trackers en webcam-tracking. (15 gecureerd — nog **13** meer in de [uitgebreide index](categories/vr.md#uitgebreide-index-13))
...
DataStores, serialisatie, compressie, state en big numbers. (141 gecureerd — nog **305** meer in de [uitgebreide index](categories/data.md#uitgebreide-index-305))
...
Quests, dialogen, input, trading, admin, signals en meer. (243 gecureerd — nog **1961** meer in de [uitgebreide index](categories/systems.md#uitgebreide-index-1961))
...
Hashing, encryptie, logins, permissies, rate limiting en anticheat. (35 gecureerd — nog **236** meer in de [uitgebreide index](categories/security.md#uitgebreide-index-236))
...
Luau-in-Luau, emulators, DOOM, N64, video, Python-in-Luau en game-ports. (147 gecureerd — nog **99** meer in de [uitgebreide index](categories/wild.md#uitgebreide-index-99))
...
Open Cloud, web-API's, Discord, databases en analytics. (131 gecureerd — nog **830** meer in de [uitgebreide index](categories/apis.md#uitgebreide-index-830))
...
MCP-servers, AI-agents en skills die in Studio kunnen bouwen. (69 gecureerd — nog **364** meer in de [uitgebreide index](categories/aitools.md#uitgebreide-index-364))
...
Runtimes, sync-tools, package managers, asset-pipelines, CI en docs. (387 gecureerd — nog **859** meer in de [uitgebreide index](categories/tooling.md#uitgebreide-index-859))
...
Handige plugins voor bouwen, terrain, UI en workflow. (92 gecureerd — nog **368** meer in de [uitgebreide index](categories/plugins.md#uitgebreide-index-368))
...
Artikelen, complete open-source games, templates en curated lijsten. (180 gecureerd — nog **933** meer in de [uitgebreide index](categories/learn.md#uitgebreide-index-933))
