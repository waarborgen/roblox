# Next-level / experimenteel

Luau-in-Luau, emulators, DOOM, N64, video, Python-in-Luau en game-ports.

## Handgecureerd (147)

| Repo | ⭐ | Wat het doet |
|---|---:|---|
| [bliporg/blip](https://github.com/bliporg/blip) | 284 | Blip: cross-platform scriptbare game-engine en platform (Luau) |
| [iErcann/NotBlox](https://github.com/iErcann/NotBlox) | 184 | Roblox-achtige multiplayer game-engine in Three.js/Node.js met auto's en physics |
| [vanyle/vectarine](https://github.com/vanyle/vectarine) | 135 | Game-engine voor snel prototypen met Luau-scripting |
| [Manonox/GDLuau](https://github.com/Manonox/GDLuau) | 105 | Luau-bindings voor de Godot Engine |
| [roblox-csharp/roblox-cs](https://github.com/roblox-csharp/roblox-cs) | 101 | C#-naar-Luau transpiler |
| [MaximumADHD/Source2Roblox](https://github.com/MaximumADHD/Source2Roblox) | 91 | Source Engine-maps (Half-Life/CS) automatisch naar Roblox porten |
| [MaximumADHD/Rbx2Source](https://github.com/MaximumADHD/Rbx2Source) | 88 | Roblox-assets naar Source Engine compileren |
| [orpos/kaledis](https://github.com/orpos/kaledis) | 71 | Luau gebruiken met LÖVE (2D-framework) |
| [MaximumADHD/Super-Nostalgia-Zone](https://github.com/MaximumADHD/Super-Nostalgia-Zone) | 65 | Broncode van Super Nostalgia Zone (oude Roblox nagebouwd) |
| [kosuke14/vLuau](https://github.com/kosuke14/vLuau) | 63 | Luau VM + compiler die in Luau draait (loadstring-alternatief) |
| [Fumohouse/godot-luau-script](https://github.com/Fumohouse/godot-luau-script) | 63 | Luau als scriptingtaal in Godot (GDExtension) |
| [tp-link-extender/MercuryCore](https://github.com/tp-link-extender/MercuryCore) | 63 | Self-hostbaar MMO-platform: 'bouw je eigen Roblox' |
| [RadiatedExodus/LuauCeption](https://github.com/RadiatedExodus/LuauCeption) | 56 | Luau draaien in Luau (via Wasynth/WebAssembly) |
| [jackhexed/luaup](https://github.com/jackhexed/luaup) | 56 | Lossless Luau-parser in Luau |
| [roblox-rs/roblox-rs](https://github.com/roblox-rs/roblox-rs) | 56 | Rust-bindings voor Roblox (Rust → WebAssembly → Luau) |
| [MaximumADHD/Roblox-PNG-Library](https://github.com/MaximumADHD/Roblox-PNG-Library) | 55 | PNG-bestanden lezen in Luau |
| [StayBlue/librebox-demo](https://github.com/StayBlue/librebox-demo) | 51 | Librebox: open-source, Roblox-compatibele game-engine |
| [roblox-compilers/roblox-py](https://github.com/roblox-compilers/roblox-py) | 42 | Python-naar-Luau compiler |
| [turtlesoupy/robloquake](https://github.com/turtlesoupy/robloquake) | 42 | Quake geport naar Roblox |
| [PhoenixWhitefire/PhoenixEngine](https://github.com/PhoenixWhitefire/PhoenixEngine) | 41 | Game-engine met Luau-scripting |
| [GameabillityOnYt/obbying-revival-project](https://github.com/GameabillityOnYt/obbying-revival-project) | 41 | Roblox-obby's nagebouwd in Godot |
| [quadigen/Kinemium-Engine](https://github.com/quadigen/Kinemium-Engine) | 40 | Sandboxed Lua-game-engine (Roblox-achtig) |
| [Atmerek/CrossplayProject](https://github.com/Atmerek/CrossplayProject) | 37 | Minecraft ↔ Roblox crossplay (beide kanten) |
| [edunad/rawrbox](https://github.com/edunad/rawrbox) | 34 | PSX-achtige game-engine met Luau-modding |
| [sircfenner/png-luau](https://github.com/sircfenner/png-luau) | 32 | PNG-bestanden lezen/schrijven in Luau |
| [Scythe-Technology/luau-roblox](https://github.com/Scythe-Technology/luau-roblox) | 30 | Roblox place/model-bestanden bewerken vanuit Luau |
| [metauni/metaboard](https://github.com/metauni/metaboard) | 30 | Multiplayer tekenborden om kennis te delen (educatie) |
| [OMouta/PyLua](https://github.com/OMouta/PyLua) | 27 | Python-interpreter embedded in Luau (Python draaien in Roblox) |
| [KumoCorp/shingetsu](https://github.com/KumoCorp/shingetsu) | 27 | Rust-native Lua/Luau implementatie |
| [uniquadev/LuauVM](https://github.com/uniquadev/LuauVM) | 26 | Luau-bytecode interpreter in Luau |
| [roblox-aurora/zirconium](https://github.com/roblox-aurora/zirconium) | 26 | Runtime scriptingtaal voor Roblox |
| [ActualMasterOogway/Iridium](https://github.com/ActualMasterOogway/Iridium) | 23 | Luau bytecode-toolkit/deserializer in Luau |
| [Nicell/lynx](https://github.com/Nicell/lynx) | 23 | Webframework in Luau (websites bouwen met Lune/Lute) |
| [KazuOfficial/Roblox-Unity](https://github.com/KazuOfficial/Roblox-Unity) | 22 | Roblox nagebouwd in Unity |
| [optimisticside/luaul](https://github.com/optimisticside/luaul) | 21 | Luau-compiler geschreven in Luau |
| [MaximumADHD/cage-mesh-deformer](https://github.com/MaximumADHD/cage-mesh-deformer) | 19 | Reverse-engineering van Roblox' cage mesh deformer (layered clothing) |
| [vantoanvh/LuauParser](https://github.com/vantoanvh/LuauParser) | 18 | Volledige Luau-parser (AST + CST) in Luau |
| [secondlife/slua](https://github.com/secondlife/slua) | 18 | Luau-fork van Second Life (Luau buiten Roblox in een virtuele wereld) |
| [rbx-loom/loom](https://github.com/rbx-loom/loom) | 18 | Roblox DSL |
| [RadiatedExodus/LuauInLuau](https://github.com/RadiatedExodus/LuauInLuau) | 17 | Luau-compiler/VM in Luau (voorganger van LuauCeption) |
| [vantoanvh/LuauNES](https://github.com/vantoanvh/LuauNES) | 16 | Snelle, accurate NES-emulator in Roblox |
| [fuse-lang/fuse](https://github.com/fuse-lang/fuse) | 16 | Statisch getypeerde taal die compileert naar Lua/Luau |
| [Open-SB/OpenSB](https://github.com/Open-SB/OpenSB) | 16 | Open-source script builder |
| [wizevaxel/picguin](https://github.com/wizevaxel/picguin) | 15 | High-performance image codecs (PNG e.d.) in Luau |
| [Nicell/htmluau](https://github.com/Nicell/htmluau) | 15 | HTML-templating in Luau |
| [Heliodex/coputer](https://github.com/Heliodex/coputer) | 15 | Collaboratieve Luau-uitvoeromgeving |
| [vinterbell/cart](https://github.com/vinterbell/cart) | 14 | Embedbare, sandboxed Luau-runtime |
| [ayulang/ayu](https://github.com/ayulang/ayu) | 14 | Statisch getypeerde taal die compileert naar Luau |
| [SovereignSatellite/Fin](https://github.com/SovereignSatellite/Fin) | 14 | Lua-achtige bytecode-interpreter in Luau |
| [thecreare/CatWebCompiler](https://github.com/thecreare/CatWebCompiler) | 14 | Compileert eigen programmeertaal naar CatWeb-scripts (in-game websites) |
| [PizzaLvr49/bevy-luau](https://github.com/PizzaLvr49/bevy-luau) | 13 | Luau-scripting voor de Bevy engine |
| [ddeityies/quantum](https://github.com/ddeityies/quantum) | 13 | Quantumcomputer-simulatie in Luau |
| [mayari-org/mayari](https://github.com/mayari-org/mayari) | 12 | Adaptief web-backend framework in Luau |
| [Plasmism/RoVM](https://github.com/Plasmism/RoVM) | 12 | Eigen 32-bit virtuele computer + besturingssysteem volledig in Roblox |
| [vantoanvh/LuauDOOM](https://github.com/vantoanvh/LuauDOOM) | 11 | De originele DOOM-engine geport naar Luau |
| [mokiros/luau_term](https://github.com/mokiros/luau_term) | 11 | Terminal-emulator (xterm-achtig) in Luau |
| [vantoanvh/LuauPorts](https://github.com/vantoanvh/LuauPorts) | 11 | Ports naar Luau: LZAV, Zlib, rANS, Base85, GIF, JPEG, PNG |
| [thekingofspace/Ruzit](https://github.com/thekingofspace/Ruzit) | 11 | Luau game-engine oplossing (Rust) |
| [Luau2TS/luau2ts](https://github.com/Luau2TS/luau2ts) | 11 | Luau-naar-TypeScript compiler |
| [Fumohouse/shadowblox](https://github.com/Fumohouse/shadowblox) | 10 | Roblox DataModel/API emuleren in Godot |
| [MaximumADHD/Roblox-VMF-Toolset](https://github.com/MaximumADHD/Roblox-VMF-Toolset) | 9 | Roblox-levels porten naar Garry's Mod |
| [vanyle/ASG](https://github.com/vanyle/ASG) | 9 | Statische website-generator in Rust + Luau |
| [Word30210/luau-hangul](https://github.com/Word30210/luau-hangul) | 8 | Koreaans (Hangul) verwerken in Luau |
| [TigersUniverse/LuauSharp](https://github.com/TigersUniverse/LuauSharp) | 8 | C#-interpreter voor Luau |
| [SovereignSatellite/LuauJBC](https://github.com/SovereignSatellite/LuauJBC) | 8 | Luau-bytecode naar Java-bytecode vertalen |
| [Asleum/moonsweeper](https://github.com/Asleum/moonsweeper) | 8 | Minesweeper in MoonScript voor Roblox |
| [TR-MZ/Scratchify-Blox-2](https://github.com/TR-MZ/Scratchify-Blox-2) | 8 | Scratch-projecten in Roblox draaien |
| [yoits9090/plumber](https://github.com/yoits9090/plumber) | 7 | Nintendo 64-emulator in Roblox (R4300 + Fast3D op EditableImage) |
| [wizevaxel/pnguin](https://github.com/wizevaxel/pnguin) | 7 | Snelle PNG-library in pure Luau |
| [mrparkerlol/script-builder](https://github.com/mrparkerlol/script-builder) | 7 | Complete script-builder in één ModuleScript |
| [lode-luau/LodeRuntime](https://github.com/lode-luau/LodeRuntime) | 7 | Snelle, modulaire Luau-runtime |
| [keplerHaloxx/roblox-chess-script](https://github.com/keplerHaloxx/roblox-chess-script) | 7 | Stockfish-schaakzetten tonen in Roblox |
| [UtoECat/miniLuau](https://github.com/UtoECat/miniLuau) | 7 | Luau-broncode in twee bestanden om makkelijk te embedden |
| [SPOOKEXE/atomic-game-engine](https://github.com/SPOOKEXE/atomic-game-engine) | 7 | C++-engine met ingebedde Luau en TypeScript |
| [malice-nz/Splice](https://github.com/malice-nz/Splice) | 7 | Luau-compiler geschreven in Luau |
| [insitux/rbxts-Insitux](https://github.com/insitux/rbxts-Insitux) | 7 | S-expression scripttaal waarmee spelers games veilig modden |
| [TornadoCookie/OpenRBLX](https://github.com/TornadoCookie/OpenRBLX) | 6 | Open-source kloon van Roblox Studio/Player |
| [schphe/embrik](https://github.com/schphe/embrik) | 6 | Scriptbare server-first 2D-game-engine |
| [Yumacide/yum](https://github.com/Yumacide/yum) | 6 | Rust-achtige getypeerde taal die compileert naar Luau |
| [R-unic/RoGems](https://github.com/R-unic/RoGems) | 6 | Ruby-naar-Lua transpiler |
| [Someon1e/chess_spider](https://github.com/Someon1e/chess_spider) | 6 | Schaakengine in Roblox |
| [nrmu9/Ro2DEngine](https://github.com/nrmu9/Ro2DEngine) | 5 | Software-renderer + physics-engine op EditableImage |
| [nightcycle/editable-clothing-util](https://github.com/nightcycle/editable-clothing-util) | 5 | Klassieke clothing naar EditableImages vertalen |
| [Zytharian/RbxStargate](https://github.com/Zytharian/RbxStargate) | 5 | Stargates (werkende poorten) in Roblox, v20 |
| [magicoal-nerb/luvm](https://github.com/magicoal-nerb/luvm) | 5 | Lua 5.1 opnieuw geïmplementeerd in getypeerde Luau |
| [LUSharp/LUSharp](https://github.com/LUSharp/LUSharp) | 5 | C# compileren naar Luau |
| [C-Cody/roblox-py](https://github.com/C-Cody/roblox-py) | 5 | Nog een Python-naar-Luau compiler |
| [pickaxe828/LynxScript-ts](https://github.com/pickaxe828/LynxScript-ts) | 5 | Tekstprogrammeertaal voor CatWeb |
| [boatbomber/Decant](https://github.com/boatbomber/Decant) | 4 | Decompressie in pure Luau |
| [Pimpoli/GodotLuau](https://github.com/Pimpoli/GodotLuau) | 4 | Luau in Godot 4, programmeren zoals in Roblox Studio |
| [not-nullptr/Ropen-Backend](https://github.com/not-nullptr/Ropen-Backend) | 4 | Open-source, reverse-engineered Roblox-backend |
| [littensy/logify](https://github.com/littensy/logify) | 4 | Minimalistische logic-gate simulator |
| [R-unic/ion](https://github.com/R-unic/ion) | 4 | Statisch getypeerde taal die naar Luau compileert |
| [alphafantomu/Love-Universe](https://github.com/alphafantomu/Love-Universe) | 4 | Roblox-achtige object-environment engine voor LÖVE |
| [Sezei/ff-kate-engine](https://github.com/Sezei/ff-kate-engine) | 4 | Modding-engine voor Funky Friday |
| [astomih/sinen](https://github.com/astomih/sinen) | 4 | Creative coding framework voor Luau |
| [TechEverythingTikTok/RobloxCPU](https://github.com/TechEverythingTikTok/RobloxCPU) | 4 | CPU-emulator in Roblox |
| [thecreare/polytoria-welds](https://github.com/thecreare/polytoria-welds) | 4 | Roblox-achtige destructie in Polytoria via grafen |
| [pon331/qr-luau](https://github.com/pon331/qr-luau) | 3 | QR-codes genereren in Luau |
| [decompi/Pixel-Stream-Player](https://github.com/decompi/Pixel-Stream-Player) | 3 | Video afspelen in Roblox: frames als pixeldata streamen via Node.js |
| [travisdmathis/quake-3-arena-engine-luau-roblox](https://github.com/travisdmathis/quake-3-arena-engine-luau-roblox) | 3 | Quake 3 Arena-engine geport naar Luau voor Roblox |
| [roblox-aurora/zirconium-ast](https://github.com/roblox-aurora/zirconium-ast) | 3 | AST-parser voor de Zirconium-DSL |
| [orblua/RobloxCore](https://github.com/orblua/RobloxCore) | 3 | Platform dat een Roblox-runtime en infrastructuur simuleert |
| [RiceTheDev/luau_cpu](https://github.com/RiceTheDev/luau_cpu) | 3 | Werkende CPU in Roblox |
| [roblox-compilers/rcc-v2](https://github.com/roblox-compilers/rcc-v2) | 3 | Roblox-compiler-collectie v2 |
| [aarontheluanerd/kanvu-prism](https://github.com/aarontheluanerd/kanvu-prism) | 3 | Browser- en WebView-engine in Roblox |
| [MrRoblick/chip8-luau](https://github.com/MrRoblick/chip8-luau) | 3 | Snelle CHIP-8 emulator |
| [Scythe-Technology/c-parser](https://github.com/Scythe-Technology/c-parser) | 3 | C-parser in Luau |
| [Zalthen-dev/Luanos-Engine](https://github.com/Zalthen-dev/Luanos-Engine) | 3 | Gratis game-development software (demo) |
| [taikonaut001/open-circuit-maker](https://github.com/taikonaut001/open-circuit-maker) | 3 | Open circuit maker (logica bouwen) |
| [Dekkonot/rbx-binary-luau](https://github.com/Dekkonot/rbx-binary-luau) | 2 | Roblox binair bestandsformaat (.rbxm) lezen in pure Luau |
| [filoxen/workspace-detector](https://github.com/filoxen/workspace-detector) | 2 | .rbxm-bestanden analyseren om oude games te vinden |
| [AnotherSubatomo/luau_qoi](https://github.com/AnotherSubatomo/luau_qoi) | 2 | QOI-afbeeldingsformaat encoder/decoder |
| [AegisLua/AegisVM](https://github.com/AegisLua/AegisVM) | 2 | Volledig sandboxed Luau-interpreter in Luau |
| [Ukuroks-games/giflib](https://github.com/Ukuroks-games/giflib) | 2 | GIFs maken in Luau |
| [shrjrd/rbxcad](https://github.com/shrjrd/rbxcad) | 2 | JSCAD (CSG/3D-modelleren in code) geport naar roblox-ts |
| [FXDuke/Lua2D](https://github.com/FXDuke/Lua2D) | 2 | 2D game-editor geïnspireerd op Roblox Studio |
| [ntech-org/Nova07](https://github.com/ntech-org/Nova07) | 2 | Clean-room herimplementatie van de Roblox-engine anno 2007 |
| [alloy-luau/alloy](https://github.com/alloy-luau/alloy) | 2 | Strikte superset van Luau die regel-voor-regel naar Luau compileert |
| [R-unic/roblox-cr](https://github.com/R-unic/roblox-cr) | 2 | Crystal-naar-Luau compiler |
| [HilthonTT/LuaScript](https://github.com/HilthonTT/LuaScript) | 2 | Lua-achtige taal met stack-VM en Luau-types, in Go |
| [wendoo-lang/wendoo-lang](https://github.com/wendoo-lang/wendoo-lang) | 2 | Tile-based visuele taal met WHEN/DO-regels |
| [sbeltranc/Kerosene](https://github.com/sbeltranc/Kerosene) | 2 | Roblox Service-API's emuleren in Ruby on Rails |
| [roblox-csharp/types](https://github.com/roblox-csharp/types) | 2 | Type-definities en generator voor Roblox C# |
| [cubacadabra/web](https://github.com/cubacadabra/web) | 2 | Browser-client van Cubacadabra (Luau-gamepackages) |
| [MrRoblick/lime-vm](https://github.com/MrRoblick/lime-vm) | 2 | Lime: een Luau virtual machine geschreven in Luau |
| [chteau/rbx-native](https://github.com/chteau/rbx-native) | 2 | Onofficiële native Studio-achtige editor in Rust (GPUI + wgpu) die .rbxl opent, rendert en bewerkt |
| [lfillaz/RobloxVideoConverter](https://github.com/lfillaz/RobloxVideoConverter) | 2 | Zet MP4 om naar part-based animaties met kleurpaletten en delta-frames |
| [DeepXx86/video-to-roblox](https://github.com/DeepXx86/video-to-roblox) | 2 | Zet MP4 om naar bit-packed frames voor EditableImage-videoplayback |
| [pon331/luau-vm](https://github.com/pon331/luau-vm) | 1 | Moderne Luau VM in Luau |
| [PhantomShift/rbx-pen-showcase](https://github.com/PhantomShift/rbx-pen-showcase) | 1 | Tablet-pen druk in Roblox via een geëmuleerde controller |
| [crownaintanoob/VideoToRobloxPlayer](https://github.com/crownaintanoob/VideoToRobloxPlayer) | 1 | Video naar frames/pixels omzetten en in Roblox tonen |
| [fak3Conde/CondeCodeStudio](https://github.com/fak3Conde/CondeCodeStudio) | 1 | Eigen game-engine + editor met Roblox-getrouwe Luau-API |
| [Asphaltian/OpenRBX](https://github.com/Asphaltian/OpenRBX) | 1 | Decompilatie en open reimplementatie van Roblox (dec. 2007) |
| [gigabyteworkstation/gta5-roblox-vehicles](https://github.com/gigabyteworkstation/gta5-roblox-vehicles) | 0 | GTA5-voertuigmeshes runtime in Roblox laden (Rust-backend + EditableMesh) |
| [pon331/unzip-luau](https://github.com/pon331/unzip-luau) | 0 | ZIP-bestanden decoderen in Luau |
| [pon331/GIFDecoder-Luau](https://github.com/pon331/GIFDecoder-Luau) | 0 | GIFs decoderen en afspelen in Roblox |
| [MingauRM/fobos-framework](https://github.com/MingauRM/fobos-framework) | 0 | Game-framework geïnspireerd op LÖVE2D en Roblox, met Luau |
| [ermak-dev/kubaria](https://github.com/ermak-dev/kubaria) | 0 | Open-source engine compatibel met Roblox, gebouwd op Godot |
| [Crazyblox/DUAUM](https://github.com/Crazyblox/DUAUM) | 0 | Luau DOOM-engine (doomgeneric), platform-agnostisch |
| [coolguy65161/QuBlox](https://github.com/coolguy65161/QuBlox) | 0 | Quantumcomputer-simulator in Luau |
| [Builder-Pals/native-legacy](https://github.com/Builder-Pals/native-legacy) | 0 | Restauratieproject voor Roblox 2006-2014 |
| [KartzRbx/CLPP](https://github.com/KartzRbx/CLPP) | 0 | CL++: C++-achtige taal die naar Luau compileert |
| [highlou/luauvm](https://github.com/highlou/luauvm) | 0 | Virtual machine met assembler en compiler voor een B-achtige taal, in roblox-ts |
| [nrbx-ts/rbxts-luau-vm](https://github.com/nrbx-ts/rbxts-luau-vm) | 0 | Loadstring-achtige Luau-VM om code uit strings in-game uit te voeren |
| [MarsSquirrel/bad-apple](https://github.com/MarsSquirrel/bad-apple) | 0 | Bad Apple afgespeeld met de Terrain-engine van Roblox |
| [TateoDev/Editable-Image-raycaster](https://github.com/TateoDev/Editable-Image-raycaster) | 0 | 3DSage-achtige raycaster-engine op EditableImage met wandtexturen |

## Uitgebreide index (390)

Automatisch verzameld en gefilterd (geen exploits/cheats/spam, geen klonen of spamfarms), categorie op trefwoorden. Omschrijving = originele GitHub-omschrijving; "(uit README)" = eerste alinea van de README (repo had geen omschrijving); "(geen omschrijving op GitHub)" = opgenomen op basis van een duidelijke reponaam.

| Repo | ⭐ | Wat het doet |
|---|---:|---|
| [mlua-rs/mlua](https://github.com/mlua-rs/mlua) | 2874 | High level Lua 5.5/5.4/5.3/5.2/5.1 (including LuaJIT) and Luau bindings to Rust with async/await support |
| [rce-incorporated/Fiu](https://github.com/rce-incorporated/Fiu) | 173 | Luau bytecode interpreter for Luau |
| [SovereignSatellite/Wasynth](https://github.com/SovereignSatellite/Wasynth) | 169 | This project has been superseded by Spider. |
| [msmalik681/Maliks-Linux-Roblox-Wrapper](https://github.com/msmalik681/Maliks-Linux-Roblox-Wrapper) | 28 | Get Roblox Up and Running and easy and simple as possible. |
| [radiantgurl/RobloxToGodotProject](https://github.com/radiantgurl/RobloxToGodotProject) | 28 | Creates an emulation layer to run Roblox games inside Godot similar to WINE without connecting to official servers. |
| [narezy/MacOBlox](https://github.com/narezy/MacOBlox) | 25 | Run the macOS Roblox client on Linux through Darling |
| [Humanagon/VirtualBloxStudio](https://github.com/Humanagon/VirtualBloxStudio) | 16 | ROBLOX Studio 2016 Emulator that runs inside Roblox! |
| [CheeseGodRoblox/RoJuicer](https://github.com/CheeseGodRoblox/RoJuicer) | 15 | This script is a simple automatic installer for all dependencies needed for Grapejuice to be installed. |
| [coneputer/robot64](https://github.com/coneputer/robot64) | 14 | WIP port of the ROBLOX game Robot 64 to Windows and HTML |
| [dreamsartificial648-sketch/Oasis-Game-Trainer](https://github.com/dreamsartificial648-sketch/Oasis-Game-Trainer) | 8 | This can be trained on Roblox, Super Mario Bros (NES), Minecraft, and whatever other games that are consistent. |
| [EmK530/Luauni](https://github.com/EmK530/Luauni) | 7 | Custom C# Luau Interpreter in development for Unity. |
| [MikeFried/Luau3D](https://github.com/MikeFried/Luau3D) | 7 | (uit README) A simple cross-platform game engine prototype using C++, Luau VM, and OpenGL. |
| [awesomefroggy29/purified](https://github.com/awesomefroggy29/purified) | 6 | Roblox runtime for arm64 Linux |
| [CorkHQ/Corky](https://github.com/CorkHQ/Corky) | 6 | The Flatpak distribution of Cork |
| [ringarang/Amp](https://github.com/ringarang/Amp) | 6 | Client modification for popular MMO game ROBLOX to allow unsigned lua script execution via the ingame VM. |
| [serasaa/RobloxOS](https://github.com/serasaa/RobloxOS) | 6 | The new open source Roblox operating system |
| [MaximumADHD/Roblox2Unity](https://github.com/MaximumADHD/Roblox2Unity) | 5 | An unfinished port of some Roblox stuff into Unity. |
| [8damon/TASX-Roblox-Optimizer](https://github.com/8damon/TASX-Roblox-Optimizer) | 4 | Multi-Instance CPU RAM Affinity Optimizer & Resource Limiter |
| [glook9001/Nuah](https://github.com/glook9001/Nuah) | 4 | Lean Linux-hosted Android compatibility runtime for Roblox |
| [juaninsanote7773/ro-cpuemu](https://github.com/juaninsanote7773/ro-cpuemu) | 4 | An open source custom-made CPU Emulator written in Luau |
| [Malte0621/RBX-CPP](https://github.com/Malte0621/RBX-CPP) | 4 | RBX-CPP allows you to program C++ in roblox with the help of emscripten and wasm2luau (from Wasynth). |
| [aitji/anti-roblox](https://github.com/aitji/anti-roblox) | 3 | do you ever hate everytime you open school computer, and found roblox on it? i don't care i am it so annoy run that exe file on dist it will put on auto start |
| [CompeyDev/luau-asm](https://github.com/CompeyDev/luau-asm) | 3 | A tiny mock assembly instruction set written in pure luau. |
| [Esternex/LUAU-Python](https://github.com/Esternex/LUAU-Python) | 3 | A python interpreter built in luau. |
| [Kampfkarren/unicode-ident-luau](https://github.com/Kampfkarren/unicode-ident-luau) | 3 | A port of dtolnay's unicode-ident crate to Luau. Check if a codepoint is XID_Start/XID_Continue |
| [LytharaLab/GFBS-Main](https://github.com/LytharaLab/GFBS-Main) | 3 | A Minecraft mod project dedicated to recreating QSERF (Roblox). |
| [MatrixPlayz4192/Roblox-Roblox-Studio-on-linux.](https://github.com/MatrixPlayz4192/Roblox-Roblox-Studio-on-linux.) | 3 | Roblox studio on linux, Vinegar Client. |
| [MicrosoftBugDispatcher/gelato-roblox](https://github.com/MicrosoftBugDispatcher/gelato-roblox) | 3 | a well written roblox external, has every feature needed and an extensive lua vm |
| [thatguyshubh/roblinx](https://github.com/thatguyshubh/roblinx) | 3 | A port for Roblox, on Linux. |
| [8ch32bit/MockSignal](https://github.com/8ch32bit/MockSignal) | 2 | A Luau module that emulates RBXScriptSignal instances (minus parallel luau functions), indended to be the fastest RBXScriptSignal implementation |
| [ChristopherAndrewTopalian/SLua_Computer_Science](https://github.com/ChristopherAndrewTopalian/SLua_Computer_Science) | 2 | In Second Life we can program using SLua. SLua enables many new possibilities in virtual world content creation. This tutorial teaches how to program using S... |
| [Crazyblox/RayLuau](https://github.com/Crazyblox/RayLuau) | 2 | [WIP] Run Luau code within a native OS window via Raylib |
| [DirtPiper/rSRC](https://github.com/DirtPiper/rSRC) | 2 | Source/Goldsrc engine emulation with a 12.17.2008 Roblox client as a base |
| [DrakeH5/Studio255-Minecraft-Roblox-Clone](https://github.com/DrakeH5/Studio255-Minecraft-Roblox-Clone) | 2 | (geen omschrijving op GitHub) |
| [emortalmc/MineDoors](https://github.com/emortalmc/MineDoors) | 2 | [Paused - Far from done] A remake of Doors from Roblox, but in Minecraft |
| [Fireboltofdeath/rbx-sqlite3full-server](https://github.com/Fireboltofdeath/rbx-sqlite3full-server) | 2 | Full access SQL api from roblox. |
| [FryerTuck152/Kageyoshi-Noir](https://github.com/FryerTuck152/Kageyoshi-Noir) | 2 | Universal OS script for roblox |
| [Heiroic/rbxminecraft](https://github.com/Heiroic/rbxminecraft) | 2 | roblox client optimizer with mc textures |
| [hihityyy/dotfiles](https://github.com/hihityyy/dotfiles) | 2 | (uit README) My personal configuration files for Arch Linux with the Niri Wayland compositor. |
| [KuyaJimbo/CodeWiz_Mini_Projects](https://github.com/KuyaJimbo/CodeWiz_Mini_Projects) | 2 | When the Python/Unity/Roblox/Minecraft Codio fails, a Computer Science Graduate from USF will prevail! |
| [SuterXOM/luau](https://github.com/SuterXOM/luau) | 2 | A Minecraft lib mod that lets other mods compile and run sandboxed Luau scripts |
| [TheFrogGod/Fiu](https://github.com/TheFrogGod/Fiu) | 2 | Luau bytecode interpreter, in Luau *MIRROR* |
| [Transmittus/Affinity](https://github.com/Transmittus/Affinity) | 2 | The Affinity Project, the first ever true Roblox-in-Roblox Luau VM. |
| [vec4me/lovlox](https://github.com/vec4me/lovlox) | 2 | Emulate Roblox games with full multiplayer support |
| [VerdommeMan/convert-to-human-readable-numbers](https://github.com/VerdommeMan/convert-to-human-readable-numbers) | 2 | A roblox lua module that converts numbers to a human readable format. (Similar to the -h option for ls in linux) |
| [vgvgvvv/Toluau](https://github.com/vgvgvvv/Toluau) | 2 | Just another Interaction layer for Roblox/luau |
| [2knoah-glitch/Linix](https://github.com/2knoah-glitch/Linix) | 1 | This is a Roblox emulator script that mimics Kali linux Debian |
| [3474nd0m/hUX2MCB](https://github.com/3474nd0m/hUX2MCB) | 1 | hopefully roblox to minecraft bus |
| [AbnormallyCursed/ro-riscv](https://github.com/AbnormallyCursed/ro-riscv) | 1 | (uit README) The actual CPU is solely by me, and everything is a work in progress, this github page may not be updated to contain the latest source code version. |
| [BinarySandia04/wasm_luau](https://github.com/BinarySandia04/wasm_luau) | 1 | Luau VM running on WASM |
| [christiancombine/Blockmango-2020-src](https://github.com/christiancombine/Blockmango-2020-src) | 1 | Source code to a minecraft and a roblox clone "blockmango" |
| [CrimsonWrecker2901/Roblox-VM](https://github.com/CrimsonWrecker2901/Roblox-VM) | 1 | (uit README) To make the Roblox virtual machine load faster, consider the following tips: |
| [drjellyjam/2d-map-module](https://github.com/drjellyjam/2d-map-module) | 1 | a 2d map module for roblox, essentially a grid with cells that can be get and set (like a minecraft chunk or something) |
| [Durpy445/AC](https://github.com/Durpy445/AC) | 1 | Port of roblox game AC to lua |
| [EnzoPontoni/LuaChokehold](https://github.com/EnzoPontoni/LuaChokehold) | 1 | Ofuscador em camadas para Lua 5.1 e Luau (Roblox): renomeação, cifra de strings, flatten de fluxo e virtualização (VM). |
| [Exunys/Roblox-Unfocused-Window-Optimization](https://github.com/Exunys/Roblox-Unfocused-Window-Optimization) | 1 | Roblox Unfocused Window Lower GPU + CPU Usage |
| [FacuCutipay/a-game-](https://github.com/FacuCutipay/a-game-) | 1 | im trying to make a game like Roblox, Minecraft, FNaF. a terror zombie survival game |
| [falsidge/emdoom-linux-binaries](https://github.com/falsidge/emdoom-linux-binaries) | 1 | binaries for my doom in roblox |
| [Fastering18/lua-brainfuck](https://github.com/Fastering18/lua-brainfuck) | 1 | Brainfuck interpreter written in lua |
| [gato1985/Computer-Roblox-DOOM](https://github.com/gato1985/Computer-Roblox-DOOM) | 1 | A port of DosBox in Roblox. |
| [GriffTDR/realtime-detector-roblox](https://github.com/GriffTDR/realtime-detector-roblox) | 1 | A Luau script that tracks the time on your computer and transfers it to your Roblox game. For example, if it's evening or night, the mode will also be dark. ... |
| [GustavoFurtad2/MinecraftCommandBlockForRoblox](https://github.com/GustavoFurtad2/MinecraftCommandBlockForRoblox) | 1 | (uit README) MCFR(Minecraft Command Block For Roblox) is a module script that adds and ports minecraft commands to roblox |
| [HuzaifaPallan/Team-Sentinals](https://github.com/HuzaifaPallan/Team-Sentinals) | 1 | A branded Prop Hunt game for Roblox and Minecraft that seamlessly integrates advertising into immersive gameplay environments using innovative gaming resourc... |
| [ikottgicki-rgb/rojoin](https://github.com/ikottgicki-rgb/rojoin) | 1 | Native Roblox client for Linux and Windows |
| [imacodr/Bloxxer](https://github.com/imacodr/Bloxxer) | 1 | Minecraft and Roblox, Connected |
| [imvickykumar999/Shawarma-Shop](https://github.com/imvickykumar999/Shawarma-Shop) | 1 | I Used SQL to Catch ANOMALIES in Roblox Shawarma Shop! |
| [inazk4zu/Hobbies](https://github.com/inazk4zu/Hobbies) | 1 | Um dos meus hobbies são jogos; como Genshin Impact, Resident Evil, Roblox e Minecraft. |
| [Jcby0/RobloxPlusPlus](https://github.com/Jcby0/RobloxPlusPlus) | 1 | (uit README) Roblox++ is a multi-os Roblox enhancement. The goal is to make your Roblox experience more enjoyable. |
| [Libre-Hacker/sunaba](https://github.com/Libre-Hacker/sunaba) | 1 | A 3D sandbox game similer to LittleBigPlanet, Minecraft, Gmod, and Roblox |
| [logancammish2/OS-Demo](https://github.com/logancammish2/OS-Demo) | 1 | Operating System demo made on Roblox |
| [lorenzogdsales-droid/GameOS](https://github.com/lorenzogdsales-droid/GameOS) | 1 | Desktop APK para Android - Relógio configurável, Eaglercraft, Roblox, WhatsApp, Panda Dome e Modo Teste com VM isolada |
| [lostyawolfer/3008](https://github.com/lostyawolfer/3008) | 1 | roblox scp-3008 recreated in minecraft |
| [lucacrose/proofreader](https://github.com/lucacrose/proofreader) | 1 | ⚡ High-speed vision pipeline for Roblox trades. Converts screenshots into structured Python dictionaries using YOLO26, CLIP, and EasyOCR. Sub-30ms latency wi... |
| [lucasmz-dev/BetterWait](https://github.com/lucasmz-dev/BetterWait) | 1 | Fork of Pyseph's RBXWait, introduces a linked list, uses DeltaTime instead of os.clock, making it similar to normal wait functions. |
| [MiguelEXE/BASIC.luau](https://github.com/MiguelEXE/BASIC.luau) | 1 | A BASIC interpreter for Roblox |
| [namooTH/roblox-hijacker](https://github.com/namooTH/roblox-hijacker) | 1 | restarts the computer if roblox is detected in the process list |
| [nefahas/roblox-16bitvm](https://github.com/nefahas/roblox-16bitvm) | 1 | roblox 16bit vm files |
| [nightcycle/instance-pool](https://github.com/nightcycle/instance-pool) | 1 | A strictly typechecked port of instance-pool |
| [PexpointMC/PolyConnect](https://github.com/PexpointMC/PolyConnect) | 1 | PolyConnect is a Polytoria + ROBLOX Connecting game where you can play on both ROBLOX and Polytoria sides, just like Minecraft and ROBLOX crossplay games! Th... |
| [plainenglishh/luau6502](https://github.com/plainenglishh/luau6502) | 1 | Series of pure-Luau emulators for the 6502 microprocessor family. |
| [pondlnwtrue007/roblox-boxing-reallife](https://github.com/pondlnwtrue007/roblox-boxing-reallife) | 1 | เล่น Untitled Boxing Game บน Roblox ด้วยการชกมวยจริงผ่านเว็บแคม — ใช้ MediaPipe Pose จับท่าทางร่างกาย (แย็บ/ฮุค/การ์ด/หลบ/ไม้ตาย) แล้วส่งปุ่มเข้าเกม / Play R... |
| [Prasoon-Rai/Roblox-Installer-Linux](https://github.com/Prasoon-Rai/Roblox-Installer-Linux) | 1 | A script for installing roblox on LInux. |
| [PythonScratcher/rod](https://github.com/PythonScratcher/rod) | 1 | Roblox On Debian 32-bit (x86) and 64-bit & Rasbian (raspberry pi OS) 32 and 64 beta |
| [quentin452/quentin452.github.io](https://github.com/quentin452/quentin452.github.io) | 1 | Portfolio — Roblox games, Minecraft mods, EvoCraft engine |
| [qwreey/qwreeyingchan-os](https://github.com/qwreey/qwreeyingchan-os) | 1 | (uit README) qwreeying-chan 프로젝트의 ququ 메니저를 이용한 개인 데스크탑(랩탑) 배포판입니다. |
| [realUjanSen/RobloxIdleTerminator](https://github.com/realUjanSen/RobloxIdleTerminator) | 1 | Automatically closes Roblox after about 20 minutes of idle disconnect to help prevent overheating, reduce CPU wear and tear, and keep your laptop from turnin... |
| [Rondom-ctrl/Script-Reverse](https://github.com/Rondom-ctrl/Script-Reverse) | 1 | Ele fará com que você faça um reverso no roblox, repetindo os mesmos movimentos só que de forma contrária |
| [RunnerTechHype/Zombie-Stories-Linux](https://github.com/RunnerTechHype/Zombie-Stories-Linux) | 1 | A Void-Based Distribution for Zombie Stories (Roblox) players |
| [sakib/MaaS](https://github.com/sakib/MaaS) | 1 | For use in private servers in the Metaverse, Minecraft, and/or Roblox |
| [ShadowDaughter/SecureLuaVirtualMachine](https://github.com/ShadowDaughter/SecureLuaVirtualMachine) | 1 | SLVM (SecureLuaVirtualMachine) is a secure Lua virtual machine for Roblox. |
| [sleepy9090/RobloxCleaner](https://github.com/sleepy9090/RobloxCleaner) | 1 | Clean Roblox junk off computer because kids. |
| [synpixel/higurashi-roblox](https://github.com/synpixel/higurashi-roblox) | 1 | A port of Higurashi When They Cry for Roblox |
| [TacticalBFG/luau-brainfuck](https://github.com/TacticalBFG/luau-brainfuck) | 1 | (uit README) this is basically a translator from brainfuck into Roblox's modified lua, luau. |
| [Tarsteel/Rodroid](https://github.com/Tarsteel/Rodroid) | 1 | Rodroid is a Mobile OS similar to Andriod coded in Lua for Roblox Game Engine. |
| [TbgeStudios/Roblox-Doors-GameBoy-Advance-Version](https://github.com/TbgeStudios/Roblox-Doors-GameBoy-Advance-Version) | 1 | A ongoing project to port the famous roblox doors to the nintendo GameBoy Advance |
| [TIMOTHY1498/RoLuaNES](https://github.com/TIMOTHY1498/RoLuaNES) | 1 | NES emulator for Roblox. |
| [tiwoah/minebloqz](https://github.com/tiwoah/minebloqz) | 1 | Minecraft recreation in Lua using Roblox game engine |
| [TNT20002220/IUBWM](https://github.com/TNT20002220/IUBWM) | 1 | A hobby project to emulate input. Designed for Breadwinner World on Roblox. |
| [Toan040606/Linux-Config](https://github.com/Toan040606/Linux-Config) | 1 | (geen omschrijving op GitHub) |
| [toughCSB/Playtime-pact](https://github.com/toughCSB/Playtime-pact) | 1 | 부모와 자녀가 정한 게임 시간 약속을 지키도록 돕는 Windows 앱. Roblox·Minecraft Java/Bedrock·Lunar Client의 Minecraft 세션을 감지해 남은 시간과 경고를 표시하고, 시간 만료 시 종료하며 부모 PIN으로 규칙과 추가 시간을 관리합니다. |
| [UGBONTOP/crunchblox-2009](https://github.com/UGBONTOP/crunchblox-2009) | 1 | A complete receation of ROBLOX in 2009 made using Unity and WASM! |
| [Valks-Forks/sunaba](https://github.com/Valks-Forks/sunaba) | 1 | A open source 3D sandbox game similer to LittleBigPlanet, Minecraft, Gmod, and Roblox |
| [VictorBolanos/IARG-OS](https://github.com/VictorBolanos/IARG-OS) | 1 | Un simulador de sistema operativo simple hecho en .luau en la herramienta/juego retro gadgets |
| [wafkse/dotfiles](https://github.com/wafkse/dotfiles) | 1 | (uit README) This is a repository containing the configuration for my personal Linux machines. |
| [0nehara/wos-fnaf](https://github.com/0nehara/wos-fnaf) | 0 | A port of Five Nights at Freddy's in the game 'Waste of Space' (Roblox). |
| [10174612-sys/123456789game](https://github.com/10174612-sys/123456789game) | 0 | make it have Roblox and minecraft |
| [1DeRBy1-alt/RobloxMinecraftAlpha](https://github.com/1DeRBy1-alt/RobloxMinecraftAlpha) | 0 | A script designed for roblox minecraft alpha 1.2.6. |
| [2md55nmxkw-arch/LuauEmu](https://github.com/2md55nmxkw-arch/LuauEmu) | 0 | A Luau script emulator that runs outside Roblox using the Lune. |
| [3069603/blockverse](https://github.com/3069603/blockverse) | 0 | BlockVerse Heroes is an NFT universe inspired by the aesthetics of Minecraft, Lego and Roblox. |
| [3lqv/Blox-Craft](https://github.com/3lqv/Blox-Craft) | 0 | Simple Roblox & Minecraft game showcase site. |
| [6y57uhhi/README](https://github.com/6y57uhhi/README) | 0 | Hello my name is Erick I play Rec Room Roblox And Minecraft Thank you for read me |
| [a01635117-pixel/frontlines-roblox](https://github.com/a01635117-pixel/frontlines-roblox) | 0 | Frontlines: Roblox (Luau) port of OpenFront. Code AGPL-3.0, maps and art CC BY-SA 4.0. |
| [aarontheluanerd/LunarOS-9-DevKit](https://github.com/aarontheluanerd/LunarOS-9-DevKit) | 0 | DevKit & Special OS edition intended for third-party developers to use for development of programs in LunarOS 9. (Roblox OS) |
| [AbnormallyCursed/i386-RBLX](https://github.com/AbnormallyCursed/i386-RBLX) | 0 | i386 emulation in ROBLOX |
| [adnankbrti-netizen/getdatascripte-roblox.github.io](https://github.com/adnankbrti-netizen/getdatascripte-roblox.github.io) | 0 | Hello! Anyone looking for a great experience on all Roblox maps and Minecraft? Yes, you're in the right place! Here you'll find Lua scripts and Minecraft res... |
| [adnankbrti-netizen/getdatascripte.github.io](https://github.com/adnankbrti-netizen/getdatascripte.github.io) | 0 | Hello! Anyone looking for a great experience on all Roblox maps and Minecraft? Yes, you're in the right place! Here you'll find Lua scripts and Minecraft res... |
| [alazemghiath18-coder/Blocks-verse](https://github.com/alazemghiath18-coder/Blocks-verse) | 0 | It's a game we're it ms mixed with Minecraft fortnite and roblox it's naive |
| [alexander-michaelides/Rofunge93](https://github.com/alexander-michaelides/Rofunge93) | 0 | Graphical Befunge93 interpreter ported to Roblox |
| [alezzzzzzzzzzzio/Minecraft-Tycoon-Map](https://github.com/alezzzzzzzzzzzio/Minecraft-Tycoon-Map) | 0 | A Minecraft Map But With roblox s Style Tycoon! |
| [alhill01-dot/yaygames](https://github.com/alhill01-dot/yaygames) | 0 | a app you can play games on but you have to buy things like Roblox Minecraft and stuff like that |
| [Ameeristan2/roneiete](https://github.com/Ameeristan2/roneiete) | 0 | minecraft roblox fortnitethis game is made by a noobie sodont leavea bad rewiew idk how to code |
| [andikasihotangchanel-hub/robict](https://github.com/andikasihotangchanel-hub/robict) | 0 | Game dunia kota Roblox gabung minecraft |
| [andres-luau/IB_IA_Computer-Science_2026](https://github.com/andres-luau/IB_IA_Computer-Science_2026) | 0 | This are the scripts of the roblox game "Untitled Learning Game". |
| [andressolanobaez-beep/PROYECT-FABRI-OS](https://github.com/andressolanobaez-beep/PROYECT-FABRI-OS) | 0 | un proyecto de python con colaboracion de Andres_la33 (nombre de roblox) hecho en python |
| [AnotherSubatomo/luxn](https://github.com/AnotherSubatomo/luxn) | 0 | An Uxn virtual machine, and a Varvara Ordinator, written in Luau. |
| [Aqwe3213/roblox2006-2013](https://github.com/Aqwe3213/roblox2006-2013) | 0 | i found this old version on my brothers old computer |
| [archlemist/cone-without-sending-stuff-bCk](https://github.com/archlemist/cone-without-sending-stuff-bCk) | 0 | Cone - Minecraft 1.8.9 texture pack to Roblox converter (AGPL-3.0) |
| [Aruissi/RedstoneGuard](https://github.com/Aruissi/RedstoneGuard) | 0 | RedStoneGuard like the one from minecraft |
| [AryanSrivastava2008/Game](https://github.com/AryanSrivastava2008/Game) | 0 | I like games like minecraft, cs:go, valorant,gta V, roblox, among us,etc. |
| [Aulas-Colmeia/Jogos-Roblox](https://github.com/Aulas-Colmeia/Jogos-Roblox) | 0 | Links com scripts para os jogos do Youtube do Simplicode |
| [AZ9tumas/block-placement-rbx](https://github.com/AZ9tumas/block-placement-rbx) | 0 | basic minecraft block placement system but in roblox |
| [bartramj025070/moonsnake](https://github.com/bartramj025070/moonsnake) | 0 | Luau interpreter, written in Python |
| [BatteRaquette581/ZINCEmulator](https://github.com/BatteRaquette581/ZINCEmulator) | 0 | funny roblox game emulator |
| [Bavar1an/Accelerated-Lua-VM](https://github.com/Bavar1an/Accelerated-Lua-VM) | 0 | Started as a personal project to beat out the Roblox Lua VM in speed, now I will continue development on this until I just can't do it anymore. |
| [BestGiter/Rendezvous-Computer-Maker](https://github.com/BestGiter/Rendezvous-Computer-Maker) | 0 | This is my Rendezvous server for my game I am moving to python called Computer Maker! It used to be on Roblox but now it is here! |
| [bigdevboss/luau2malbolge](https://github.com/bigdevboss/luau2malbolge) | 0 | (uit README) A Malbolge toolkit written in Luau. Includes a spec-correct VM, a code generator that produces Malbolge programs to print strings, an interpreter, a tracer, ... |
| [BlazeBreeze/blazebreezeplays.github.io](https://github.com/BlazeBreeze/blazebreezeplays.github.io) | 0 | BlazeBreezePlays 🎮 / Gaming (Minecraft, Fortnite, Roblox) / Video Editing 🎬 / Digital Marketing 📈 / Coding 💻 |
| [blitz-rojo/NexusOS-Free](https://github.com/blitz-rojo/NexusOS-Free) | 0 | Roblox Nexus OS - Free version |
| [blondedkar/Roffinity](https://github.com/blondedkar/Roffinity) | 0 | rework of my previous drawing project, a professional-grade CPU rasterizer. |
| [BloxSQL/SQLBlox](https://github.com/BloxSQL/SQLBlox) | 0 | A project link SQL to roblox |
| [blueskychan-dev/Roblox-Studio-Operating-System](https://github.com/blueskychan-dev/Roblox-Studio-Operating-System) | 0 | ROBLOX OPERATING SYSTEM SOURCE CODE |
| [bradmessenger/luau-double-bits](https://github.com/bradmessenger/luau-double-bits) | 0 | (uit README) Routines for manipulating binary representations of a IEEE 754 double precision numbers. A port of double-bits. |
| [brysti/ROBLOX-y-sus-beneficios-en-los-ni-os](https://github.com/brysti/ROBLOX-y-sus-beneficios-en-los-ni-os) | 0 | aqui veras que habilidades desarrollan tus hijos al jugar roblox |
| [buscariolo-rogerio/portifolio-robloxiano](https://github.com/buscariolo-rogerio/portifolio-robloxiano) | 0 | (uit README) Conheça os 5 desenvolvedores amantes de roblox, que juntos no ensino médio técnico em Desenvolvimento de Sistemas desenvolveram esses sites. |
| [Buteca/Felipe-](https://github.com/Buteca/Felipe-) | 0 | Jogo Minecraft e Roblox |
| [CarlosHip1900/bits-host-panel](https://github.com/CarlosHip1900/bits-host-panel) | 0 | A good control panel for minecraft, roblox or other servers |
| [Caty15/Castle-Wars-Mini-Game](https://github.com/Caty15/Castle-Wars-Mini-Game) | 0 | A Minecraft Minigame based on BedWars and the Roblox game "Doomspire Brickbattle" |
| [CavefulGames/os-locale-lune](https://github.com/CavefulGames/os-locale-lune) | 0 | Get os locale in lune |
| [ccxmIcal/js-like-lang-luau-interpreter](https://github.com/ccxmIcal/js-like-lang-luau-interpreter) | 0 | this is an interpreter written in roblox luau for a js like lang invented by me (i was bored send help) |
| [cgfvdd548/Robloxfree](https://github.com/cgfvdd548/Robloxfree) | 0 | (uit README) Get a clean, ready-to-go Linux box in seconds. |
| [ch1x0r/RobloxOS](https://github.com/ch1x0r/RobloxOS) | 0 | This is a Linux OS that has Roblox configured |
| [chester119-dev/RSQL-2.0](https://github.com/chester119-dev/RSQL-2.0) | 0 | An adaptation of SQL for Roblox Studio. |
| [Chillu4/mruby.luau](https://github.com/Chillu4/mruby.luau) | 0 | Implementation of mruby's virtual machine in Luau |
| [cizmer/CareerCompass](https://github.com/cizmer/CareerCompass) | 0 | Project for 2026 FBLA Computer Game and SImulation |
| [cl1ents/Box3D-Roblox-Experiment](https://github.com/cl1ents/Box3D-Roblox-Experiment) | 0 | (uit README) Box3D compiled to WebAssembly, then translated with Spider for Luau/LuaJIT experiments. |
| [Coconutwater21/deadfall-roblox](https://github.com/Coconutwater21/deadfall-roblox) | 0 | Roblox (Luau) port of Deadfall, a top-down zombie survival game — from-scratch reimplementation, not a compile of the Flutter original. |
| [codeSmith12/CFF_Windows_Setup](https://github.com/codeSmith12/CFF_Windows_Setup) | 0 | Follow these instructions to set up the windows machines for Roblox/Minecraft activities. |
| [cubacadabra/desktop](https://github.com/cubacadabra/desktop) | 0 | our mac windows and linux player |
| [curtjustin1527-dev/Curt-](https://github.com/curtjustin1527-dev/Curt-) | 0 | I love Minecraft and Roblox |
| [Dak4008/oofdeath](https://github.com/Dak4008/oofdeath) | 0 | i love birds and to game on roblox and minecraft |
| [Dani-Gaming/tec](https://github.com/Dani-Gaming/tec) | 0 | TECENS is a Roblox project made by TEC. We empower the creation of the ultimate experience in operating systems and all in one computers. This repo contains ... |
| [Darkdemon137v/Dark](https://github.com/Darkdemon137v/Dark) | 0 | I'm a Roblox/Minecrafter gamer |
| [DiamondRedstone/RoCraft](https://github.com/DiamondRedstone/RoCraft) | 0 | ROBLOX and Minecraft Mixed together |
| [diyya381-afk/roblox-bridge-To-VM-xD](https://github.com/diyya381-afk/roblox-bridge-To-VM-xD) | 0 | bridging tiny10 onto a rblx vm HHEEHHE |
| [DJKuhleb/Roblox_BlockWorldCreation](https://github.com/DJKuhleb/Roblox_BlockWorldCreation) | 0 | Created to simulate world creation similar to minecraft. Unfinished |
| [doqedev/ro-utils](https://github.com/doqedev/ro-utils) | 0 | A roblox utility program made for Linux |
| [drozakkacper-gif/GameWiki](https://github.com/drozakkacper-gif/GameWiki) | 0 | Nowoczesna strona internetowa z kodami, poradnikami i pro tipami do gier takich jak CS2, Minecraft, War Thunder, Roblox i wielu innych. |
| [dta995787-svg/DTA-gameing](https://github.com/dta995787-svg/DTA-gameing) | 0 | ROBLOX And minecraft player who makes video |
| [Dued-Dev/Number-guessing-game-](https://github.com/Dued-Dev/Number-guessing-game-) | 0 | So i coded a game in PyCharm where you simply guess a game, and no, this is NOT inspired of a Roblox game., where the computer generates a random integer bet... |
| [DustAtom-dev/dOS_Luau](https://github.com/DustAtom-dev/dOS_Luau) | 0 | (uit README) dOS_Luau or "dOS" is an Operating System (OS) written entirely in Pilot.lua, a programming language derived from Roblox Luau (rbx.lua) and made specifically ... |
| [Dylan2408R2/vamos-segundo-intento-de-crossplay](https://github.com/Dylan2408R2/vamos-segundo-intento-de-crossplay) | 0 | Intentamos de nuevo el crossplay de minecraft y roblox |
| [ednetymattos-tech/Mega-Knight](https://github.com/ednetymattos-tech/Mega-Knight) | 0 | EU AMO ROBLOX STUDIO ROBLOX NORMAL PROGRAMAÇÃO E MINECRAFT (E CLASH ROYALE) |
| [edzvuh2025-ai/operix](https://github.com/edzvuh2025-ai/operix) | 0 | The Operating System for Roblox Groups |
| [eiejshsjskeue838-hue/Script-1](https://github.com/eiejshsjskeue838-hue/Script-1) | 0 | Script de roblox que quando e executado todos os jogadores para mim ficam com uma aura vermelha que tem como ver atravéz das paredes também |
| [elfrangin/kryon-bridge](https://github.com/elfrangin/kryon-bridge) | 0 | Roblox <-> Minecraft Bridge API |
| [Emirgithuprezerve/Turkish-Minecraft-mod-and-Roblox-game-discovery-project.-](https://github.com/Emirgithuprezerve/Turkish-Minecraft-mod-and-Roblox-game-discovery-project.-) | 0 | “Minecraft mod ve Roblox oyun keşif uygulaması” |
| [emortalmc/BeastEscape](https://github.com/emortalmc/BeastEscape) | 0 | A Minecraft remake of Flee the Facility from Roblox |
| [emtee40/roblox-wrapper-vala](https://github.com/emtee40/roblox-wrapper-vala) | 0 | [WIP] A port of the Roblox Linux Wrapper to the Vala programming language. Should be cross-platform, and is most definitely a work in progress. |
| [ErdemProHaHaHack/minecraft-apks](https://github.com/ErdemProHaHaHack/minecraft-apks) | 0 | By ErdemPro655 in youtube. In Roblox, ErdemPro655_3. In MC ErdemPro655 |
| [etilton/ROL-patches](https://github.com/etilton/ROL-patches) | 0 | Patches related to Roblox on Linux. |
| [evoincorp/doom](https://github.com/evoincorp/doom) | 0 | The funny DOOM RCE! |
| [ewd3v/Plates-of-Doom](https://github.com/ewd3v/Plates-of-Doom) | 0 | A rework of my old Plates of Doom script |
| [exvisible/2016-March-port-of-Nebula-SG](https://github.com/exvisible/2016-March-port-of-Nebula-SG) | 0 | This is specifically for 2016M< clients only going as far to 2015M I believe. |
| [F0gzin/ObsidianMC](https://github.com/F0gzin/ObsidianMC) | 0 | A Java Virtual Machine + LWJGL emulator made on Roblox |
| [Felix010985/luau86](https://github.com/Felix010985/luau86) | 0 | Virtual CPU working in Roblox |
| [Felixdogde/Pyp-ass--multiclient-DE-](https://github.com/Felixdogde/Pyp-ass--multiclient-DE-) | 0 | Ein Multitool um besser in spiele zu sein wie z.m.b : Fortnite, Minecraft, Roblox. (Und ein spaß gegenüber euren Freunden). |
| [fgabrieltop212/Script-](https://github.com/fgabrieltop212/Script-) | 0 | Um script De roblox Para todos os jogos da platadorma |
| [frqstbite/roblox-minecraft](https://github.com/frqstbite/roblox-minecraft) | 0 | More-or-less a proof of concept that Minecraft-like features can be performantly implemented in Roblox |
| [fttamatar/Super-Go](https://github.com/fttamatar/Super-Go) | 0 | A Super Go : A game inspired on Minecraft and Roblox |
| [gab2chocolate2/Gabriel-Araujo](https://github.com/gab2chocolate2/Gabriel-Araujo) | 0 | I like roblox, minecraft, baldi's basics game, Super mario bros games, making video games. |
| [GAMBLING-MAN/BasicCodeInterpreter](https://github.com/GAMBLING-MAN/BasicCodeInterpreter) | 0 | The code of the Basic Code Interpreter module script on Roblox. |
| [gameslegalzito-oss/Caixa-de-Scripts-do-Roblox](https://github.com/gameslegalzito-oss/Caixa-de-Scripts-do-Roblox) | 0 | Uma Caixa com tabelas de todos os Scripts para você usar no Roblox |
| [gghalos/Roblox-Studio-Mod-Manager-Linux](https://github.com/gghalos/Roblox-Studio-Mod-Manager-Linux) | 0 | Port of MaximumADHD's Roblox Studio Mod Manager to work with Vinegar on Linux. |
| [gingerphoenix10/Ro32](https://github.com/gingerphoenix10/Ro32) | 0 | Allows roblox games to have more control over your computer. Unsafe and experimental |
| [GuestY7T/METAVERSE](https://github.com/GuestY7T/METAVERSE) | 0 | 🎮 METAVERSE - A production-ready 3D gaming platform with 50+ features, mini-games, avatar customization, social systems, and PWA support. Inspired by Roblox,... |
| [GuHamYaiXD/TickRate](https://github.com/GuHamYaiXD/TickRate) | 0 | Minecraft's Tickrate in Roblox |
| [guimarkcanalhao/Os-Robloxos](https://github.com/guimarkcanalhao/Os-Robloxos) | 0 | Humilhar tudo e todos os grupos |
| [GuiXvier/Mastering-Roblox-Coding](https://github.com/GuiXvier/Mastering-Roblox-Coding) | 0 | Repositório para os exercícios do livro Matering Roblox Coding |
| [Gund4422/BF-interpreter-in-luau](https://github.com/Gund4422/BF-interpreter-in-luau) | 0 | a interpreter for Brainf-ck in luau |
| [gustavogordinbr-spec/Roblox](https://github.com/gustavogordinbr-spec/Roblox) | 0 | (uit README) Um mundo de fantasia que se quebrou em ilhas suspensas depois de uma tempestade de energia antiga. Cada região muda conforme os jogadores exploram, limpam fe... |
| [Gusthavo-Henrick/Project99NightsAsm](https://github.com/Gusthavo-Henrick/Project99NightsAsm) | 0 | It's a game inspired in 99 Nights made in roblox, so i decided to make it in version of assembly. |
| [handyhandsanitiser/ps1-luau](https://github.com/handyhandsanitiser/ps1-luau) | 0 | What would go wrong making a ps1 emulator in a singlethreaded language? |
| [Helloburp/luau-locke-nes](https://github.com/Helloburp/luau-locke-nes) | 0 | (geen omschrijving op GitHub) |
| [herobrine10pro/preguntas-con-if](https://github.com/herobrine10pro/preguntas-con-if) | 0 | //= declaramos (le ponemos el valor a la variable) //== preguntamos si algo es true o false let minecraft = "pro" console.log(minecraft=="pro"); console.log(... |
| [hewel/dotfiles](https://github.com/hewel/dotfiles) | 0 | (uit README) My personal Linux dotfiles managed by chezmoi. |
| [HeyTiagoBloxOficialYt-Dev/index.html](https://github.com/HeyTiagoBloxOficialYt-Dev/index.html) | 0 | 🌙 Descrição para o site Bem-vindo ao site oficial do Heyy_TGzinBloxOfcYt 🌙🎮 Aqui você encontra os links oficiais do meu canal do YouTube e da minha conta no ... |
| [horihori2013/MCCrossRoblox](https://github.com/horihori2013/MCCrossRoblox) | 0 | An experimental project that let roblox users to join Minecraft servers that are based on paper. |
| [i2si2s/Robloc](https://github.com/i2si2s/Robloc) | 0 | Découvrez notre nouveau jeu un mélange de Minecraft fortnite roblox |
| [iandavemelleza02-alt/Speach-](https://github.com/iandavemelleza02-alt/Speach-) | 0 | Minecraft and roblox speach |
| [idklol69-12/Pls124-roblox-OS](https://github.com/idklol69-12/Pls124-roblox-OS) | 0 | it is a custom OS but it is in beta |
| [IFMNB/Hexagon](https://github.com/IFMNB/Hexagon) | 0 | Open source simple library to emulate Roblox/java construction. |
| [ifnotnithya/The-Minecraft-Roblox-ultimate-reprosity](https://github.com/ifnotnithya/The-Minecraft-Roblox-ultimate-reprosity) | 0 | This will Teach people about Minecraft / Roblox. |
| [ikii12/roblox-hand-controller](https://github.com/ikii12/roblox-hand-controller) | 0 | (uit README) Project ini adalah skrip Python yang menggunakan Computer Vision untuk mengontrol pergerakan karakter di game Roblox menggunakan gestur jari tangan. Dibuat m... |
| [imskyyc/RoL-patchset-dev](https://github.com/imskyyc/RoL-patchset-dev) | 0 | Roblox-on-Linux patchsets for wine 9.0+ |
| [isaac-gesiman/projeto-para-reconhecer-o-movimento-dos-olhos-e-boca](https://github.com/isaac-gesiman/projeto-para-reconhecer-o-movimento-dos-olhos-e-boca) | 0 | Como o Roblox faz para saber quando o nosso rosto se movimenta, pisca, e fala e aplica isso nos nossos avatares do Roblox? por conta disso criei em Python um... |
| [IsEmil/emulator](https://github.com/IsEmil/emulator) | 0 | This is a pretty simple emulator for metatables on Roblox |
| [itriedcoding/roblox-linux-installer](https://github.com/itriedcoding/roblox-linux-installer) | 0 | Play Roblox on ANY Linux distro. Full installer with setup for Ubuntu, Debian, Fedora, Arch, and 15+ more distros. |
| [jeancarlocontact-tech/invitacion-cumple-gamer](https://github.com/jeancarlocontact-tech/invitacion-cumple-gamer) | 0 | Invitación cumpleaños gamer estilo Minecraft y Roblox - Maximiliano 8 años |
| [joaopaulodf52/roblox-](https://github.com/joaopaulodf52/roblox-) | 0 | (uit README) Este repositório contém os scripts principais para um jogo estilo RPG no Roblox. O projeto está organizado para separar responsabilidades do servidor, client... |
| [Joaoplims/sna_roblox_kg](https://github.com/Joaoplims/sna_roblox_kg) | 0 | (uit README) Este repositório contém os notebooks e scripts Python utilizados para a construção, tratamento e análise topológica da rede social do Roblox, desenvolvidos c... |
| [JungyoKim/Roblox_Python_Interpreter](https://github.com/JungyoKim/Roblox_Python_Interpreter) | 0 | 로블록스 Lua 웹어셈블리 기반 Python Interpreter를 위한 서버 / 2025 |
| [jyotiradityaBiswas/PFS-16-Bit-Computer-Roblox](https://github.com/jyotiradityaBiswas/PFS-16-Bit-Computer-Roblox) | 0 | This is a fully open source repository, which is a 16 bit computer simulated in roblox using real logic gates and binary. Updates are not expected to be regu... |
| [kaikeend/godot-luau-bridge](https://github.com/kaikeend/godot-luau-bridge) | 0 | High-performance Luau VM integration core module for Godot 4. |
| [kaydenfuchs683-dev/IronBro-Fan-Club-](https://github.com/kaydenfuchs683-dev/IronBro-Fan-Club-) | 0 | Join the IronBro26 Fan Club for Minecraft & Roblox videos, fun tutorials, exclusive games, and the latest fan content! |
| [kdolag/Adverse](https://github.com/kdolag/Adverse) | 0 | Advertise you game for free. Be it your first ROBLOX game or a Minecraft server or what ever party you want! |
| [KentangKnox/Roblox-plyr](https://github.com/KentangKnox/Roblox-plyr) | 0 | you want a roblox in computer/laptop come here and download but sry this is are some virus |
| [Kheshwen/Roblox-Logic-Gate](https://github.com/Kheshwen/Roblox-Logic-Gate) | 0 | (uit README) An interactive, 3D educational environment for Roblox designed to teach players the fundamentals of computer science logic gates. |
| [kingdudely/LuauInJS](https://github.com/kingdudely/LuauInJS) | 0 | A Luau VM in JavaScript |
| [kingdudely/small-vm](https://github.com/kingdudely/small-vm) | 0 | A stack based virtual machine written in Luau |
| [kirdaaa/luau-brainfuck](https://github.com/kirdaaa/luau-brainfuck) | 0 | (uit README) This interpreter was targeted at performance, a major optimization is merging continuous symbols into single instruction (example: ++++++ > +6). |
| [Kloodi2021/blocky-worlds-engine-roblox](https://github.com/Kloodi2021/blocky-worlds-engine-roblox) | 0 | This is an open-source free engine that is for roblox, Allowing you to make minecraft-inspired games. |
| [kortsolosv12/Roblox-Unblocked](https://github.com/kortsolosv12/Roblox-Unblocked) | 0 | An unblocked version of Roblox (could technically run on a Chromebook but you'd need Linux) |
| [kotoba-lang/com-roblox](https://github.com/kotoba-lang/com-roblox) | 0 | (uit README) Clean-room API-compatible implementation of the roblox platform, backed by Datomic and Py Kotodama WASM. |
| [krigerofc/combo](https://github.com/krigerofc/combo) | 0 | # Combos_prontos_roblox Esste Script é feito para um jogo de boxe do roblox, onde ele aperta as teclas do seu teclado e faz com que os combos e tecnicas seja... |
| [kroesufos/Blocky](https://github.com/kroesufos/Blocky) | 0 | a dumb project to try to join minecraft and roblox in a server sided addon |
| [kvmuralidharan/aarush-](https://github.com/kvmuralidharan/aarush-) | 0 | Hi l'm aarush and l love Minecraft Subway surfers roblox and all other |
| [LattaAi09/RoCraft](https://github.com/LattaAi09/RoCraft) | 0 | A combination of Minecraft and Roblox |
| [Lays0chips/roblox-financial-analysis](https://github.com/Lays0chips/roblox-financial-analysis) | 0 | Python, SQL, and Tableau analysis of Roblox 10-Q SEC filings comparing Q1 2024, Q1 2025, and Q1 2026 |
| [LboY752/modelforge](https://github.com/LboY752/modelforge) | 0 | great modeling app meant for all needs roblox models, movie models, minecraft models, real life costume models, etc with this app you can create anything |
| [Lbozololni/Gyco-OPT-V5.3](https://github.com/Lbozololni/Gyco-OPT-V5.3) | 0 | Gyco OPT V5.3 - Top 1 optimization for: Roblox, minecraft, CS2 and other games! |
| [leandrofts/RobloxEMetaverso](https://github.com/leandrofts/RobloxEMetaverso) | 0 | Repositório para os desafios do curso Roblox e Metaverso da DIO.me |
| [leonnatale/luau-wasm](https://github.com/leonnatale/luau-wasm) | 0 | Run Luau with WebAssembly |
| [lillianrubyrose/mrow](https://github.com/lillianrubyrose/mrow) | 0 | Automated Arch Linux system setups with LuaU or TOML à la bad nix |
| [lindapargan-max/operix](https://github.com/lindapargan-max/operix) | 0 | The Operating System for Roblox Groups - Full Stack SaaS |
| [loderplayz/loderplayz.github.io](https://github.com/loderplayz/loderplayz.github.io) | 0 | Site officiel de Kyro (LoderPlayz) — créateur YouTube Minecraft et Roblox |
| [logancammish/os-demo](https://github.com/logancammish/os-demo) | 0 | Operating System Demo is a Roblox experience in which users can simulate an operating system-like environment |
| [lohguecit33/ovaautojoinroblox](https://github.com/lohguecit33/ovaautojoinroblox) | 0 | untuk menjalakan roblox dan membuka game secara otomatis di emulator |
| [loocea69/turnbinds-wayland](https://github.com/loocea69/turnbinds-wayland) | 0 | This is an application written in C to bring turnbinds (just like AHK) to Wayland on Linux :) |
| [lordnhoj1/Inio-DOS](https://github.com/lordnhoj1/Inio-DOS) | 0 | fake lightweight operating system for a custom luau console |
| [lordnhoj1/LF3XEmulatorBasics](https://github.com/lordnhoj1/LF3XEmulatorBasics) | 0 | a set of functions for the lua microchip in roblox game "Logically F3X" |
| [Loscope/Assembly-Libraries](https://github.com/Loscope/Assembly-Libraries) | 0 | Not Assembly Assembly but Roblox external |
| [lovingminecraft7-sketch/RioyTube-](https://github.com/lovingminecraft7-sketch/RioyTube-) | 0 | RioyTube je rovnaké ako YouTube. Ale môže sa tam ukladať fotky & videá! A čo je najlepšie? Microsoft & Google account! RioyTube by lovingminecraft. Minecraft... |
| [luisfenadovillanueva891-cpu/Robloxhubxray](https://github.com/luisfenadovillanueva891-cpu/Robloxhubxray) | 0 | Scrippara Roblox Minecraft xray |
| [lumpylegscausebad/Computer-Science-NEA-Code---Nicolas-Mills](https://github.com/lumpylegscausebad/Computer-Science-NEA-Code---Nicolas-Mills) | 0 | This contains all of my code for my project |
| [Lunar-Orbit1/JoinAssistant](https://github.com/Lunar-Orbit1/JoinAssistant) | 0 | A small program built on python, made to run on Windows (and eventually MacOS and Linux distros) that will help you automatically configure your join setting... |
| [lunar-xyz/RobloxVirtualMachine](https://github.com/lunar-xyz/RobloxVirtualMachine) | 0 | (geen omschrijving op GitHub) |
| [Maklit-Productions/roblox-on-linux-patch](https://github.com/Maklit-Productions/roblox-on-linux-patch) | 0 | (uit README) This Roblox wrapper was designed for new and beginner Linux users to quickly get Roblox up and running as easy as possible. |
| [malice-nz/Caesar](https://github.com/malice-nz/Caesar) | 0 | Run Roblox's x86 Android client natively on windows! |
| [marceloangelon0-sys/marcelo](https://github.com/marceloangelon0-sys/marcelo) | 0 | faça um script local lua roblox quando eu quicar ç todos os jogadores ficarem com um hightlight vermelho |
| [Matheus-Romling/Templates-luau](https://github.com/Matheus-Romling/Templates-luau) | 0 | Só um local pra guardar os códigos que venho fazendo |
| [Melvin-The-Goat/Melvin-Portfolio-2026](https://github.com/Melvin-The-Goat/Melvin-Portfolio-2026) | 0 | Professional portfolio BS/ Blended MS Computer Science student at University of the Pacific. Showcasing Game Development (Unity, Godot, Roblox) and Software ... |
| [mgzinn425-design/mgzinnZ6K.github.io](https://github.com/mgzinn425-design/mgzinnZ6K.github.io) | 0 | Site oficial do mgzinn — Criador de conteúdo gamer focado em Roblox, lives, desafios e projetos insanos. Todos os links oficiais reunidos aqui. |
| [mimilouprime70/Obby-crevasse-Beta-1-](https://github.com/mimilouprime70/Obby-crevasse-Beta-1-) | 0 | To see how the game is made, open the attached files (make sure you have Roblox Studio installed on your computer) |
| [mindsyon/robloxdoubleclap](https://github.com/mindsyon/robloxdoubleclap) | 0 | (uit README) Run: just open the python script. Double clap and it opens roblox (bloxstrap, fishtrap included and etc) make sure your microphone in windows or any other OS... |
| [MINTILER-DEV/MentuxOS](https://github.com/MINTILER-DEV/MentuxOS) | 0 | (uit README) Mentux OS is a guest virtual computer implemented inside Roblox Studio. Luau is only the host language that emulates the hardware stack. |
| [MixInforBrazil/Arcade-Craft](https://github.com/MixInforBrazil/Arcade-Craft) | 0 | Este servidor foi feito pela intenção de criar uma comunidade de minecraft e roblox. |
| [mohammadhussain9824-cloud/Jbsbdhdhsdjm](https://github.com/mohammadhussain9824-cloud/Jbsbdhdhsdjm) | 0 | I love minecraft for houses for teacher and my favourite game to name is roblox |
| [mokiros/luau_gameboy_emulator](https://github.com/mokiros/luau_gameboy_emulator) | 0 | (uit README) The purpose of this project is not to write an accurate emulator, but rather research ways to optimize Luau code to be efficient and fast. |
| [molgang/schatveld](https://github.com/molgang/schatveld) | 0 | Schatveld Weddewarden — schatgraven/boer/politie in het Land-Wursten marschveld (Roblox + Minecraft-datapack) |
| [MortyMo22/Minecraft-Roblox-Edition-](https://github.com/MortyMo22/Minecraft-Roblox-Edition-) | 0 | (geen omschrijving op GitHub) |
| [mos1983/Roblox-Computer-Store-Sim](https://github.com/mos1983/Roblox-Computer-Store-Sim) | 0 | (geen omschrijving op GitHub) College Project |
| [MrMarcelss/ROBLOX-STUDIO](https://github.com/MrMarcelss/ROBLOX-STUDIO) | 0 | Aqui os dejo el archivo para descargar ROBLOX STUDIO. Una app de ROBLOX para crear tus propios videojuegos de ROBLOX |
| [MrMayestic/CST-project](https://github.com/MrMayestic/CST-project) | 0 | Computer Store Tycoon Game on Roblox, built with Lua and Roblox Studio IDE. |
| [mrnickzh/Chip8EmulatorRoblox](https://github.com/mrnickzh/Chip8EmulatorRoblox) | 0 | just a CHIP-8 emulator, but in roblox :/ |
| [MrSprinkleToes/chip8-interpreter-luau](https://github.com/MrSprinkleToes/chip8-interpreter-luau) | 0 | (uit README) By default, the project is set up to run a Breakout game program, then shortly after reset the interpreter and load a heartbeat monitor program. |
| [MyMel2001/roblox-scrcpy](https://github.com/MyMel2001/roblox-scrcpy) | 0 | Streaming ROBLOX from an Android phone to a Linux PC, and controlling it. All with the help of scrcpy appimage. |
| [new-qwertyui/RobloxTranslationLayer](https://github.com/new-qwertyui/RobloxTranslationLayer) | 0 | Playing Roblox Bedwars in Minecraft 1.8.9 |
| [nightcycle/maid](https://github.com/nightcycle/maid) | 0 | typed port of Quenty's maid class |
| [Nonescripter-rblx/VapeV4ForMinerscave](https://github.com/Nonescripter-rblx/VapeV4ForMinerscave) | 0 | Best script for roblox minecraft (aka Minerscave) |
| [nooblox601/nooblox-world-xbox-series-s-edition-its-free](https://github.com/nooblox601/nooblox-world-xbox-series-s-edition-its-free) | 0 | jogo de roblox e minecraft ao mesmo tempo |
| [notlbock/PartOS](https://github.com/notlbock/PartOS) | 0 | partos is a roblox virtual computer |
| [OfficialValkyrie/Luau-Recreation](https://github.com/OfficialValkyrie/Luau-Recreation) | 0 | Recreating Luau in Luau |
| [optimisticside/rounix](https://github.com/optimisticside/rounix) | 0 | An emulator for a UNIX-based operating system within Roblox (API wise) |
| [outerfaith/moonstone](https://github.com/outerfaith/moonstone) | 0 | A Minecraft: Java Edition compatible server written in Luau - not affiliated with Minecraft or Mojang AB |
| [Patrocinio0001/waspalaengine](https://github.com/Patrocinio0001/waspalaengine) | 0 | WaspalaEngine - Complete Luau VM, sUNC execution environment, and reconstructor |
| [pawprint69femboy/-](https://github.com/pawprint69femboy/-) | 0 | ╭─── ✨ ───╮ 🐾 𝒫𝒶𝓌𝒫𝓇𝒾𝓃𝓉 🤍 🎂 18 / 🇪🇬 / ♂️ ╰─── ✨ ───╯ ─── ⋆⋅☆⋅⋆ ─── 🎮 Gaming: ✧ VRChat & Roblox 🌌 ✧ PUBG & COD 🔫 ✧ Minecraft ⛏️ ─── ⋆⋅☆⋅⋆ ─── 🌍 En / Ar 💬 |
| [pedr-ui/Divine-store](https://github.com/pedr-ui/Divine-store) | 0 | Melhor loja de Roblox, com várias contas e coisas sobre os ovos, contado com game passe tudo |
| [pereiragenildo740-droid/Game-pass](https://github.com/pereiragenildo740-droid/Game-pass) | 0 | Game Pass Premium – A melhor experiência dentro do jogo! ⭐ Todos os benefícios do VIP ⭐ Bônus exclusivos ⭐ Poderes especiais Compra 100% oficial pela platafo... |
| [phapsugoldapple-jpg/the1v1game](https://github.com/phapsugoldapple-jpg/the1v1game) | 0 | Currently, it only has Minecraft vs Roblox; I will add more later. |
| [piroleidejacintopiroca-cmd/minecraft-server-roblox](https://github.com/piroleidejacintopiroca-cmd/minecraft-server-roblox) | 0 | (geen omschrijving op GitHub) |
| [PlungerMC/DOORS-Refurbished---Bedrock](https://github.com/PlungerMC/DOORS-Refurbished---Bedrock) | 0 | DR is an addon wich tries to recreate the roblox experience "DOORS" but in minecraft |
| [qasimovsid0-creator/-](https://github.com/qasimovsid0-creator/-) | 0 | Сайт-Представляем вам сайт про 88 блогеров тематики "Minecraft,Roblox" - Некоторые блогеры могут быть удалены Удачи ))) |
| [qasimovsid0-creator/Blogers-Wiki](https://github.com/qasimovsid0-creator/Blogers-Wiki) | 0 | Сайт-Представляем вам сайт про 88 блогеров тематики "Minecraft,Roblox" - Некоторые блогеры могут быть певцами Удачи ))) |
| [QNTM-Labs/QStudio](https://github.com/QNTM-Labs/QStudio) | 0 | roblox studio for linux |
| [Qwe-Notorious/MCEngine-Studi-Dev](https://github.com/Qwe-Notorious/MCEngine-Studi-Dev) | 0 | The mod for minecraft fabric 1.21.1 JDK 21 is similar to Roblox Studio only immediately with the execution of all commands in the interior of the game. |
| [R-unic/create-mod](https://github.com/R-unic/create-mod) | 0 | Create mod from Minecraft but for Neanderthals |
| [R0bl0x10501050/CSS-Interpreter-In-RLua](https://github.com/R0bl0x10501050/CSS-Interpreter-In-RLua) | 0 | This won't run correctly in Repl as it's written is a dialect of Lua called Roblox Lua (Rlua). |
| [r3x0n44/Reach-Universal-for-Roblox](https://github.com/r3x0n44/Reach-Universal-for-Roblox) | 0 | Esse é um Reach disponivel para todos os tipos de jogos no Roblox! |
| [raphtalia/Signal](https://github.com/raphtalia/Signal) | 0 | (uit README) Emulation of Roblox signals with extended features. |
| [raymaud-hub/pws-english-gaming](https://github.com/raymaud-hub/pws-english-gaming) | 0 | PWS: Learning English Through Video Games - Fortnite, Roblox, Minecraft |
| [rbxrootx/rbxoffline](https://github.com/rbxrootx/rbxoffline) | 0 | A WIP experimental Roblox Studio Emulator. |
| [Redusofficial/LuaAppRuntime](https://github.com/Redusofficial/LuaAppRuntime) | 0 | (uit README) Project aimed to emulate the 2018L (2.365.265265) Mobile app inside Roblox |
| [Renen12/robloxpauser](https://github.com/Renen12/robloxpauser) | 0 | Simple script to pause Roblox on MacOS (could easily be adjusted to work on Linux, probably.) |
| [RetroPlayDev/ClassicBlox](https://github.com/RetroPlayDev/ClassicBlox) | 0 | This is ClassicBlox, inspired by Minecraft and Roblox! |
| [RLBXos/rblxos.github.io](https://github.com/RLBXos/rblxos.github.io) | 0 | a os built inside roblox. |
| [rnathaniel45/LuVM](https://github.com/rnathaniel45/LuVM) | 0 | In-Luau virtualized Luau Bytecode Virtual Machine |
| [ROBlNET13/noob-boy-adventures-love2d](https://github.com/ROBlNET13/noob-boy-adventures-love2d) | 0 | Port of the "The Adventures of Noob Boy: Part 1" Roblox gear game for Love2D |
| [rochasamuel/roblox-projects](https://github.com/rochasamuel/roblox-projects) | 0 | (uit README) Aqui temos um projeto de um jogo no estilo "Block Party" lembrando os minigames do Minecraft. Implementei ele quando trabalhava no Simplicode. |
| [Rod2009P/Craftmine](https://github.com/Rod2009P/Craftmine) | 0 | Mauricio el aventurero de roblox en minecraft digo craftmine |
| [Romidzeez/code-of-future-2024](https://github.com/Romidzeez/code-of-future-2024) | 0 | my work on education project "Code of Future", roblox and minecraft ( lua and python ) |
| [rumblee1/machina](https://github.com/rumblee1/machina) | 0 | Machina is a host-neutral IBM PC–compatible emulator core implemented in Luau. |
| [ryanrn/tiktok-roblox-live](https://github.com/ryanrn/tiktok-roblox-live) | 0 | (uit README) Le os comentarios de uma live do TikTok em tempo real e transforma cada um em acao dentro de uma experiencia do Roblox. |
| [sa8498611-glitch/Roblox](https://github.com/sa8498611-glitch/Roblox) | 0 | Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecraft Minecra... |
| [sambojohnkevin0-jpg/John-Kevin-Sambo](https://github.com/sambojohnkevin0-jpg/John-Kevin-Sambo) | 0 | I am gaming Minecraft and roblox |
| [Sancar22-debug/Roblox_CV-School-Project](https://github.com/Sancar22-debug/Roblox_CV-School-Project) | 0 | (uit README) This is a Deep Learning project that brings real-time computer vision into Roblox. |
| [sbi-n/lluuaauu](https://github.com/sbi-n/lluuaauu) | 0 | Luau VM in luau |
| [scroder-nesise/Real-time-Clock-for-Roblox](https://github.com/scroder-nesise/Real-time-Clock-for-Roblox) | 0 | A simple clock & watch i made for roblox scripting. Uses os.clock, os.date, and os.time. enjoy watching your own time |
| [sealtastic/BrainfuckInterpreterLUAU](https://github.com/sealtastic/BrainfuckInterpreterLUAU) | 0 | prolly the smallest brainfuck interpreter in luaU (aka, roblox) |
| [secwall/valkey-luau](https://github.com/secwall/valkey-luau) | 0 | (uit README) A Luau scripting engine module for Valkey, providing EVAL, EVALSHA, SCRIPT and FUNCTION backed by an embedded Luau VM. |
| [Senko3141/AP_CSP_2024](https://github.com/Senko3141/AP_CSP_2024) | 0 | Program code for my Create Task of the AP Computer Science Principles exam. "Gradebook Program" This program uses Lua and ROBLOX Studio to create a "game", a... |
| [server222/Roblox-Linux-Semi-Nativo](https://github.com/server222/Roblox-Linux-Semi-Nativo) | 0 | Proyecto para diseñar un motor para correr roblox en linux de forma casi nativa y r |
| [sewnie/molette](https://github.com/sewnie/molette) | 0 | Simplest proof of concept Roblox wrapper |
| [shadow-deathchild/Gamer-Paradise](https://github.com/shadow-deathchild/Gamer-Paradise) | 0 | Welcome to Gamer's Paradise! We'll be covering Among us, Roblox, Minecraft and Fortnite! All the basic, advanced and experienced tips are here! |
| [ShahinGamesRewards/Games-Reward-Center-](https://github.com/ShahinGamesRewards/Games-Reward-Center-) | 0 | 🎮 Gaming Reward Center – A fun gaming reward demo where you can select Fortnite, Minecraft, Roblox or Brawl Stars rewards. Choose your game, select a reward ... |
| [shellprompt/risc-cpu](https://github.com/shellprompt/risc-cpu) | 0 | Risc-like CPU project built with logic gates on ROBLOX. |
| [SherryMaster/roblox-temporary-blob-event](https://github.com/SherryMaster/roblox-temporary-blob-event) | 0 | (uit README) Blob Solver is a computer-vision-assisted SameGame planner for a colored-block browser game. |
| [shreyasnan/Roblox-Accounts-Monitoring](https://github.com/shreyasnan/Roblox-Accounts-Monitoring) | 0 | Monitor game account listings across Eldorado.gg, U7Buy, and eBay for Roblox, Fortnite, Minecraft, and Steam |
| [SilasRotroDevTeam/RotroBeta](https://github.com/SilasRotroDevTeam/RotroBeta) | 0 | Rotro Beta is a Roblox emulator replica |
| [simontdobson/roblox-minecraft-screentime](https://github.com/simontdobson/roblox-minecraft-screentime) | 0 | (uit README) Listen for Windows processes associated with Roblox or Minecraft starting up and/or ending Setup windows scheduler task to stop process once time limit has e... |
| [sosiska-killer-of/doors_mc](https://github.com/sosiska-killer-of/doors_mc) | 0 | roblox doors in minecraft |
| [SoupsyLive/Deepwoken](https://github.com/SoupsyLive/Deepwoken) | 0 | My personal Minecraft adaptation of the Roblox Game Deepwoken from team Vows of the Sea managed by Ragoozer. |
| [spooketti/SiO2](https://github.com/spooketti/SiO2) | 0 | using computer vision to track my body to beat a roblox obby |
| [Spookyz10/RPG-project](https://github.com/Spookyz10/RPG-project) | 0 | (uit README) Common/src contém o código compartilhado por Lobbies e Raids. Edite ali para atualizar os dois places. |
| [Steve4044-qweq132123/Steve4044](https://github.com/Steve4044-qweq132123/Steve4044) | 0 | I love playing minecraft and roblox |
| [stevecodeshq/roblox-workforce-intelligence-project](https://github.com/stevecodeshq/roblox-workforce-intelligence-project) | 0 | Workforce data analysis capstone — Excel, SQL, Python, Power BI |
| [SxrothDev/ServersCx](https://github.com/SxrothDev/ServersCx) | 0 | The best page to find private Roblox servers or Minecraft servers. |
| [synpixel/brainfuck.luau](https://github.com/synpixel/brainfuck.luau) | 0 | An implementation of Brainfuck in Luau |
| [synpixel/umineko-roblox](https://github.com/synpixel/umineko-roblox) | 0 | A port of Umineko When They Cry to Roblox |
| [Tamasina07/ROBLOX-OS](https://github.com/Tamasina07/ROBLOX-OS) | 0 | A fake Roblox operating system I barely even finished. I do not remember making this at all. |
| [teachkidslab/stem-academy-site](https://github.com/teachkidslab/stem-academy-site) | 0 | Інтерактивний сайт STEM Academy з курсами програмування для дітей: Minecraft, Roblox, Scratch, Python, JavaScript, Fullstack, ШІ, Computer Science. Сучасний ... |
| [Team-Wuye/intSQL](https://github.com/Team-Wuye/intSQL) | 0 | SQL Interpreter into LuaU |
| [TechEverythingTikTok/zxOS](https://github.com/TechEverythingTikTok/zxOS) | 0 | An OS on roblox with PM and VFS (not real CPU emulated) |
| [TehnoTheDragon/avm](https://github.com/TehnoTheDragon/avm) | 0 | A Virtual Machine (Luau) |
| [Testerboy21/World-Destruction](https://github.com/Testerboy21/World-Destruction) | 0 | (uit README) A luau roblox module that emulates in-game world destruction from native BaseParts and preset models |
| [tfhrigit/robloxmove](https://github.com/tfhrigit/robloxmove) | 0 | (uit README) menggunakan deteksi gerakan tangan berbasis computer vision. Menggunakan OpenCV dan MediaPipe untuk real-time hand tracking, aplikasi ini memetakan berbagai ... |
| [tgraupmann/CSDK_ChromaRoblox](https://github.com/tgraupmann/CSDK_ChromaRoblox) | 0 | Sample C++ port of Chroma Design |
| [theafamily323-dotcom/Minecraft-DOORS-TexturePacks](https://github.com/theafamily323-dotcom/Minecraft-DOORS-TexturePacks) | 0 | this contains my Roblox DOORS texture packs! |
| [TheFreePenguin/rolex](https://github.com/TheFreePenguin/rolex) | 0 | ROBLOX FOR LINUX (not working on this anymore) |
| [TheSecondTry/RobloxLagClips](https://github.com/TheSecondTry/RobloxLagClips) | 0 | A script that tries to emulate lag clipping in Roblox. |
| [THEthingmaker/pilot.lua-OS](https://github.com/THEthingmaker/pilot.lua-OS) | 0 | Repository for my special OS in entirely made in pilot.lua, The programming language of roblox game "Waste of space". |
| [tiipolp/Roguetonin](https://github.com/tiipolp/Roguetonin) | 0 | A script for a roblox game named Rogueblox, using the Serotonin Lua VM |
| [timmybutler/RobloxEnvironmentLogger](https://github.com/timmybutler/RobloxEnvironmentLogger) | 0 | Ultimate Roblox Environment Logger - All VM and Sandbox techniques combined |
| [Timtam/osap-daw-hosts](https://github.com/Timtam/osap-daw-hosts) | 0 | (uit README) Shared DAW host-window matchers — a library module for the OS automation platform (topic osap-module). |
| [TrickyFNFexe/Mineblox](https://github.com/TrickyFNFexe/Mineblox) | 0 | Hoping to create working/stable and open source Minecraft and Roblox Crossplay. |
| [Tyskalox/Bloxventure](https://github.com/Tyskalox/Bloxventure) | 0 | Base code for one of my highly customizable roblox game, Bloxventure, which is inspired by minecraft. |
| [ultradeduction/info2](https://github.com/ultradeduction/info2) | 0 | ✩basic dni criteria,dni proshippers,12-/21+ ➶ i play overwatch 2, idv, minecraft,roblox,hsr/gi/hi3,etc ➶ INTP/♊︎/5w4✩ |
| [underpicked/ROBLOX-Run-On-Linux](https://github.com/underpicked/ROBLOX-Run-On-Linux) | 0 | This python script makes ROBLOX be able to run on linux without kicking you from the game. |
| [User-2056/tictactoe](https://github.com/User-2056/tictactoe) | 0 | Tic Tac Toe single-page app with CPU opponent |
| [vaderus/RBLXnolinux](https://github.com/vaderus/RBLXnolinux) | 0 | meu scritp para automaticamente instalar o grapejuice e rodar Roblox no Gnu/Linux |
| [vaotech/linblox](https://github.com/vaotech/linblox) | 0 | Roblox running in linux! |
| [vasitproject06/index](https://github.com/vasitproject06/index) | 0 | This site is about the subjects I love like Space, Cricket, Roblox and Minecraft |
| [vectorjack37/ProjectToolBox](https://github.com/vectorjack37/ProjectToolBox) | 0 | (uit README) ProjectToolBox: Reviving We are trying to rebuild the old Toolbox for ROBLOX versions 2006 - 2009 as if it was like you were on the old family computer! |
| [VeeDW/FAMS.os](https://github.com/VeeDW/FAMS.os) | 0 | интерфейс из roblox pleace quantum sciance energy resach facylyty |
| [victorgagababa-lgtm/roblox-folder-Version-folder-](https://github.com/victorgagababa-lgtm/roblox-folder-Version-folder-) | 0 | for people on VM's that cant install roblox. |
| [viografiko/Roblox-Sandbox](https://github.com/viografiko/Roblox-Sandbox) | 0 | Tries to emulate a roblox studio sandbox. |
| [vkh700/linblox](https://github.com/vkh700/linblox) | 0 | Launch roblox on linux systems! |
| [voidstonism/WoS-OperatorOs](https://github.com/voidstonism/WoS-OperatorOs) | 0 | Terminal Based OS for WoS |
| [VoidusX/sparrow](https://github.com/VoidusX/sparrow) | 0 | A minimal, Lua/Luau oriented, customizable linux distro with batteries included. |
| [Vovchik830/Roblox-Doom](https://github.com/Vovchik830/Roblox-Doom) | 0 | Working copy of Doom in Roblox. |
| [vttt1903-create/GUIDO](https://github.com/vttt1903-create/GUIDO) | 0 | сайт с гайдами по популярным играм! 🎮 Здесь ты найдёшь полезные советы, инструкции и подсказки для Minecraft, Brawl Stars, Roblox, Don’t Starve Together и др... |
| [walksanatora/wasm_hell](https://github.com/walksanatora/wasm_hell) | 0 | i wanted to compile rust to roblox why wont it let me :wah: |
| [wallissonghost-code/Robloxjogo](https://github.com/wallissonghost-code/Robloxjogo) | 0 | (uit README) Jogo imobiliário no Roblox: compre propriedades, receba aluguel, melhore os imóveis, venda seus ativos e dispute o maior patrimônio da cidade. |
| [Warhead51707/3008-Addon](https://github.com/Warhead51707/3008-Addon) | 0 | Recreation of the SCP 3008 game on Roblox for Minecraft. |
| [weakassauce/tower-defense-roblox](https://github.com/weakassauce/tower-defense-roblox) | 0 | Roblox Luau port of the Three.js tower defense prototype |
| [WeatherGeek1/QuantOS](https://github.com/WeatherGeek1/QuantOS) | 0 | This is a Linux distro, based on Ubuntu, that is based on the fictional QuantOS from the Roblox game Quantum Science Energy Research Facility (QSERF for shor... |
| [webstylepress/aPrincessPlays](https://github.com/webstylepress/aPrincessPlays) | 0 | (uit README) Welcome to aPrincessPlays! Fun and family-friendly gaming videos channel in Urdu / Hindi 🎉 I play popular games like Roblox, Minecraft, Palworld, Stardew Val... |
| [whatiscodee/99nights-minecraft](https://github.com/whatiscodee/99nights-minecraft) | 0 | roblox 99 nights game in minecraft :) |
| [whoodaphonech/Ant-](https://github.com/whoodaphonech/Ant-) | 0 | I like Video games most of the time i play "Roblox or Minecraft" i will choose when ever i feel like playing that game |
| [wizevaxel/chip8-luau](https://github.com/wizevaxel/chip8-luau) | 0 | A relatively fast Luau CHIP-8 interpreter & emulator |
| [wordunathan-stack/Doom-Fruits](https://github.com/wordunathan-stack/Doom-Fruits) | 0 | A roblox mmo game based on one of the best animes |
| [wrexlua/luavm](https://github.com/wrexlua/luavm) | 0 | Lua Virtual Machine for Luau Programing Language |
| [wyattgiraudon4-lgtm/Bloxncraft-](https://github.com/wyattgiraudon4-lgtm/Bloxncraft-) | 0 | Un mélange entre roblox et minecraft |
| [xdx-sf/XTech-1](https://github.com/xdx-sf/XTech-1) | 0 | The XTech-1 engine is the game engine developed by the XDX Software Foundation to recreate Doom games on Roblox. |
| [yeon-11/roblox-rocket-game](https://github.com/yeon-11/roblox-rocket-game) | 0 | Roblox rocket assembly game with body swap mechanics |
| [zacharyceiley-jpg/happy-home-of-robloxia](https://github.com/zacharyceiley-jpg/happy-home-of-robloxia) | 0 | WebAssembly Port of Sonic Mania Plus |
| [zachthearcticfox/Arctic-Fox-16-Assembler](https://github.com/zachthearcticfox/Arctic-Fox-16-Assembler) | 0 | Assembler for a work-in-progress CPU built in a Roblox logic simulator "Circuit maker 2" |
| [ZAL-X/Roblox_Minecraft_bow_fight_PROC-31](https://github.com/ZAL-X/Roblox_Minecraft_bow_fight_PROC-31) | 0 | (geen omschrijving op GitHub) |
| [zaz-zy/rbx-linux-runtime](https://github.com/zaz-zy/rbx-linux-runtime) | 0 | Unofficial Wine-based compatibility research for Roblox on Linux. Not affiliated with Roblox Corporation. |
| [zenithhhhchannhhhh/nowayCAS](https://github.com/zenithhhhchannhhhh/nowayCAS) | 0 | nowayCAS is a Computer Algebra System made for Roblox Studio. |
| [Zuphrus/Unnamed-Computer-Core-archive](https://github.com/Zuphrus/Unnamed-Computer-Core-archive) | 0 | Its public domain - gmhs |

[← Terug naar overzicht](../README.md)
